...
/** The main-actor facade for scrollback history.
 ...
 and forwards storage work to `ScrollbackClient`. */
@MainActor
final class Scrollback {
	...
	private let client: ScrollbackClient
	...
	init(client: ScrollbackClient = .shared) {
		self.client = client
	}
	...
	@discardableResult
	func indexLogLine(_ logLine: LogLine, forView viewIdentifier: String) -> Bool {
	...
	func indexLogLines(_ logLines: [LogLine], forView viewIdentifier: String) {
	...
	func writeNewEntry(with logLine: LogLine, forView viewIdentifier: String) {
	...
	func writeNewEntry(_ entry: ScrollbackEntry, for logLine: LogLine) {
