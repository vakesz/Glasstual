...
nonisolated enum InlineImageContainer { // nonisolated: value
	struct Animation {
		let frames: Int
		let decodedBytes: Int
	}

	static func animation(in data: Data, limits: InlineImageLimits) throws -> Animation? {
	...
	private static func gifFrames(_ bytes: UnsafeRawBufferPointer,
	                              limits: InlineImageLimits) throws -> Animation
	{
		guard bytes.count >= 13 else { throw InlineImageError.unsupportedContent }
		...
		while offset < bytes.count {
			...
			offset += 1
			if marker == 0x3B {
				guard frames > 0 else { throw InlineImageError.unsupportedContent }
				return Animation(frames: frames, decodedBytes: footprint * frames)
			}
			if marker == 0x2C {
				guard bytes.count - offset >= 9 else { throw InlineImageError.unsupportedContent }
				...
				else { throw InlineImageError.resourceLimit }
				frames += 1
			...
			while true {
				guard offset < bytes.count else { throw InlineImageError.unsupportedContent }
				let length = Int(bytes[offset])
				offset += 1
				guard length <= bytes.count - offset else { throw InlineImageError.unsupportedContent }
				offset += length
				if length == 0 {
					break
				}
	                              ...
	                              limits: InlineImageLimits) throws -> Animation?
	{
		...
		while bytes.count - offset >= 12 {
			...
			guard length <= bytes.count - offset - 12 else { throw InlineImageError.unsupportedContent }
			let type = integer(at: offset + 4)
			switch type {
			case 0x4948_4452: // IHDR
				guard offset == 8, length == 13 else { throw InlineImageError.unsupportedContent }
			...
			case 0x6163_544C: // acTL
				guard length == 8, declaredFrames == nil,
				      footprint > 0 else { throw InlineImageError.unsupportedContent }
			...
			case 0x6663_544C: // fcTL
				guard length == 26, let declaredFrames,
				      frames < declaredFrames else { throw InlineImageError.unsupportedContent }
