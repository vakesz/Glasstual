...
struct HighlightsPane: View {
	...
	private var matchingMethod: NicknameHighlightMatchMode {
		model.settings[SettingsKeys.Highlights.matchingMethod]
	}
	...
	var body: some View {
		Section {
			Picker(selection: model.settings.binding(for: SettingsKeys.Highlights.matchingMethod)) {
			...
			SettingsToggle(
				...
				isOn: model.settings.binding(for: SettingsKeys.Logging.logHighlights)
			)
			SettingsToggle(
				title: .Settings.highlightsTrackLocalNickname,
				isEnabled: usesRegularExpression == false,
				isOn: model.settings.binding(for: SettingsKeys.Highlights.trackLocalNickname)
			)
		}
		...
		Section {
			SettingsKeywordList(
				...
				keywords: model.settings.binding(for: SettingsKeys.Highlights.matchKeywords),
			...
			SettingsKeywordList(
				...
				keywords: model.settings.binding(for: SettingsKeys.Highlights.excludeKeywords)
			)
