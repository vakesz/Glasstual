...
/// The Connection row: how much the session asks of a server at once, and what
...
struct ConnectionPane: View {
	...
	var body: some View {
		...
		Section {
			SettingsToggle(
				...
				isOn: model.settings.binding(for: SettingsKeys.Messages.replyToCTCPRequests)
			)
			SettingsToggle(
				title: .Settings.incomingDataHighlightSpam,
				note: .Settings.incomingDataHighlightSpamNote,
				isOn: model.settings.binding(for: SettingsKeys.Messages.detectHighlightSpam)
			)
			SettingsToggle(
				title: .Settings.incomingDataRemoveFormatting,
				note: .Settings.incomingDataRemoveFormattingNote,
				isOn: model.settings.binding(for: SettingsKeys.Messages.removeAllFormatting)
			)
			SettingsToggle(
				title: .Settings.incomingDataUnicodeSpam,
				note: .Settings.incomingDataUnicodeSpamNote,
				isOn: model.settings.binding(for: SettingsKeys.Messages.filterUnicodeTextSpam)
			)
		} header: {
	...
	private var delayRow: some View {
		let value = model.settings.binding(
			for: SettingsKeys.Connection.autojoinDelayAfterIdentification
		)
	...
	private var channelSizeRow: some View {
		let value = model.settings.sliderBinding(
			for: SettingsKeys.Appearance.trackUserAwayStatusMaximumChannelSize
		)
		return SettingsSliderRow(
