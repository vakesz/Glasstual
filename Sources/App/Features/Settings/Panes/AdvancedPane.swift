...
struct AdvancedPane: View {
	...
	private var logsToDisk: Bool {
		model.settings[SettingsKeys.Logging.logToDisk]
	}
	...
	var body: some View {
		Section {
			SettingsToggle(
				...
				isOn: model.settings.binding(for: SettingsKeys.Logging.logToDisk)
			)
		...
		Section {
			SettingsToggle(
				...
				isOn: model.settings.invertedBinding(for: SettingsKeys.Internals.appSleepDisabled)
			)
			SettingsToggle(
				title: .Settings.hiddenLoadHistoryLazily,
				isOn: model.settings.binding(for: SettingsKeys.Logging.loadHistoryLazily)
			)
			SettingsToggle(
				title: .Settings.hiddenSidebarTranslucency,
				isOn: model.settings.invertedBinding(
					for: SettingsKeys.Appearance.disableSidebarTranslucency
				)
			)
			scrollbackLimitRow
		} header: {
	...
	private var scrollbackLimitRow: some View {
		LabeledContent {
			SettingsComboField(
				...
				value: model.settings.numberField(for: SettingsKeys.Logging.scrollbackVisibleLimit),
