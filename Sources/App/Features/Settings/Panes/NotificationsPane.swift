...
struct NotificationsPane: View {
	...
	private var playsSound: Binding<Bool> {
		let muted = model.settings.binding(for: SettingsKeys.Notifications.soundIsMuted)
	...
	var body: some View {
		Section {
			SettingsToggle(
				...
				isOn: model.settings.binding(for: SettingsKeys.Notifications.notifyAboutMentions)
			)
		...
		Section {
			SettingsToggle(
				...
				isOn: model.settings.binding(for: SettingsKeys.Notifications.displayDockBadge)
			)
			SettingsToggle(
				title: .Settings.notificationsDockBadgePublic,
				isOn: model.settings.binding(for: SettingsKeys.Notifications.publicMessageCountOnDockBadge)
			)
		} header: {
		...
		Section {
			SettingsToggle(
				...
				isOn: model.settings.binding(for: SettingsKeys.Notifications.postWhileInFocus)
			)
		} header: {
