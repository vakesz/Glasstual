...
enum FileTransferDialogAction: Sendable {
	case start
	case stop
	case remove
	case open
...
@Observable
final class FileTransferDialogModel {
	private(set) var transfers: [TDCFileTransferDialogTransferController] = []
	...
	var visibleTransfers: [TDCFileTransferDialogTransferController] {
		_ = presentationRevision
		return filter.shownTransfers(in: transfers, isSender: \.isSender)
	}

	var selectedTransfers: [TDCFileTransferDialogTransferController] {
		transfers.filter { selection.contains($0.uniqueIdentifier) }
	}

	var stoppedTransfers: [TDCFileTransferDialogTransferController] {
		transfers.filter { Self.stoppedStatuses.contains($0.transferStatus) }
	}

	var activeTransfers: [TDCFileTransferDialogTransferController] {
	...
	func add(_ transfer: TDCFileTransferDialogTransferController) {
		transfers.insert(transfer, at: 0)
		refreshPresentation()
	}

	func remove(_ removedTransfers: [TDCFileTransferDialogTransferController]) {
	...
	func transfers(with identifiers: Set<String>) -> [TDCFileTransferDialogTransferController] {
	...
	func canPerform(_ action: FileTransferDialogAction, on identifiers: Set<String>? = nil) -> Bool {
	...
	private static func hasLocalFile(_ transfer: TDCFileTransferDialogTransferController) -> Bool {
...
struct FileTransferRowPresentation {
	...
	init(transfer: TDCFileTransferDialogTransferController) {
	...
	private static func activeStatus(
		for transfer: TDCFileTransferDialogTransferController,
		totalSize: String
	) -> String {
