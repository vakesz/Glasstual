...
enum ChannelBanListEntryType: UInt {
	case ban = 0
	case banException
...
struct ChannelBanListEntry: Identifiable, Hashable, Sendable {
	let id = UUID()
	var entryMask = ""
...
struct ChannelBanListComparator: SortComparator {
	...
	func compare(_ lhs: ChannelBanListEntry, _ rhs: ChannelBanListEntry) -> ComparisonResult {
...
final class ChannelBanListModel {
	var entries: [ChannelBanListEntry] = []
	var selection: Set<ChannelBanListEntry.ID> = []
	...
	var entryCountDescription: String {
		ChannelBanListStrings.entryCount(
			entries.count,
	...
	var truncationNotice: String? {
		discardedEntryCount > 0 ? ChannelBanListStrings.truncationNotice : nil
	}

	func add(_ entry: ChannelBanListEntry) {
	...
	private func insertionIndex(for entry: ChannelBanListEntry) -> Int {
	...
	private static func compare(
		_ lhs: ChannelBanListEntry,
		_ rhs: ChannelBanListEntry,
...
@MainActor
@Observable
final class ChannelBanListWindowState {
	var session: ChannelBanListSession?
}
...
@MainActor
@Observable
final class ChannelBanListSession: ChannelScoped {
	let client: Client
	let channel: Channel
	let entryType: ChannelBanListEntryType
	let clientId: String?
	...
	init?(entryType: ChannelBanListEntryType, in channel: Channel) {
	...
	var heading: String {
		ChannelBanListStrings.heading(for: entryType, channelName: channel.name)
	}
	...
	/** Whether a reply `client` sent for `channelName` under `modeSymbol` is
	 ...
	 channel. */
	func matches(client: Client, channelName: String, modeSymbol symbol: String) -> Bool {
