...
enum AppearanceColor: Decodable, Equatable, Sendable {
	...
	var color: NSColor? {
		...
		case let .calibratedRGB(red, green, blue, alpha):
			NSColor.calibratedColor(red: red, green: green, blue: blue, alpha: alpha)
	...
	static let systemColors: [String: NSColor] = [
...
enum AppearanceStatefulColor: Decodable, Equatable, Sendable {
	...
	func color(forActiveWindow isActive: Bool) -> NSColor? {
		switch self {
...
struct AppearanceSize: Decodable, Equatable, Sendable {
	let width: Double
	let height: Double
...
enum AppearanceSchema {
	...
	static func load<Schema: Decodable>(
...
class ApplicationAppearance {
	...
	init(applicationProperties: AppearancePropertyCollection) {
		self.applicationProperties = applicationProperties
	}

	/// The appearance snapshot the application is drawing in right now.
	@MainActor
	static var currentApplicationProperties: AppearancePropertyCollection {
		AppServices.appearance.properties
	}
