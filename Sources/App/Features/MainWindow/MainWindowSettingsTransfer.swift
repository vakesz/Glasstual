...
/** Importing and exporting the settings archive from the main window.
 ...
 owns one of these and hands it to a modifier; `SettingsTransferSession` is
 shared with the Settings window, which is why the host is stamped before the
 panel opens. */
@MainActor
@Observable
final class MainWindowSettingsTransferModel {
	let session = SettingsTransferSession.shared
...
private struct MainWindowSettingsTransferPresentation: ViewModifier {
	@Bindable var model: MainWindowSettingsTransferModel
...
extension View {
	func settingsTransfer(_ model: MainWindowSettingsTransferModel) -> some View {
		modifier(MainWindowSettingsTransferPresentation(model: model))
	}
}
