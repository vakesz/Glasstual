...
final class PreferencesPaneModel {
	...
	/// Read only the app's persisted domain: the effective defaults value also
	/// includes global languages, which must still appear as System Default.
	var appLanguage: AppLanguage {
		get {
			_ = preferences[Preferences.Internals.appLanguages]
			let domain = UserDefaults.standard.persistentDomain(forName: ApplicationInfo.applicationBundleIdentifier())
			return AppLanguage(override: domain?[Preferences.Internals.appLanguages.name] as? [String])
		}
		set {
	...
	func refreshIRCv3Connections() {
		ircv3Connections = AppServices.world.clientList.map { client in
