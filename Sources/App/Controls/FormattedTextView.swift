...
class FormattedTextView: NSTextView, NSTextViewDelegate, CustomKeyboardEventResponder {
	...
	func prepareInitialState() {
		...
		if SettingsKeys.Messages.rightToLeftFormatting.value {
	...
	/** Replacing the whole field, not editing it.
	 ...
	 being thrown away wholesale -- switching conversation replaces the field's
	 contents, and the next Undo replayed the previous conversation's ranges against
