...
extension ServerSession {
	...
	private func handleChannelModeNumeric(_ message: Message) {
		...
		guard modeString != "+", let channel = findConversation(message.params[1]) else { return }
		if channel.isActive {
	...
	private func handleTopicNumeric(_ message: Message) {
		guard message.params.count == 3, let channel = findConversation(message.params[1]) else { return }
	...
	private func handleTopicMetadataNumeric(_ message: Message) {
		guard message.params.count == 4, let channel = findConversation(message.params[1]),
		      shouldPrintReceivedMessage(message, withText: nil, destinedFor: channel)
		else { return }
		let setter = message.params[2].nicknameFromHostmask
	...
	private func handleInvitingNumeric(_ message: Message, shouldPrint: Bool) {
		guard shouldPrint, message.params.count == 3, let channel = findConversation(message.params[2]) else { return }
		print(
	...
	private func handleISONNumeric(_ message: Message, shouldPrint: Bool) {
		...
		for directConversation in conversationList where directConversation.isDirect {
			let foldedName = casefoldNickname(directConversation.name)
			guard asked.contains(foldedName) else { continue }
			applyPresence(foldedOnline.contains(foldedName), to: directConversation)
		}
	}
	...
	private func handleWHONumeric(_ message: Message, shouldPrint: Bool) {
		...
		guard let channel = findConversation(message.params[1]) else { return }
	...
	private func handleWHOXNumeric(_ message: Message, shouldPrint: Bool) {
		...
		guard let channel = findConversation(message.params[2]) else { return }
		receiveWhoReply(in: channel, reply: WHOReply(
			...
			account: IdentityPolicy.account(fromWireValue: message.params[7]),
			updatesAccount: true
		))
	}
	...
	private func handleNamesNumeric(_ message: Message, shouldPrint: Bool) {
		...
		guard let channel = findConversation(message.params[2]), !channel.channelNamesReceived else { return }
	...
	private func addName(_ rawName: String, to channel: Conversation) {
		var modes = ""
		...
		let parsed = hostmask.hostmask(on: self)
		...
		/* The NAMES reply is the server's own list, so its prefixes are the
		 truth about every name in it — including one the session already holds a
		...
		var editedMember = userAssociated(user, with: channel)
			?? Member(user: user, prefixes: currentUserPrefixes)
	...
	private func handleEndOfNamesNumeric(_ message: Message, shouldPrint: Bool) {
		...
		guard let channel = findConversation(message.params[1]), !channel.channelNamesReceived else { return }
	...
	private func handleModeListNumeric(_ numeric: ServerNumeric, message: Message, shouldPrint: Bool) {
		...
		let hasModeLetterField = numeric == .quietlist &&
			message.params.count > 3 && message.params[2].isModeSymbol
		let offset = hasModeLetterField ? 1 : 0
		let mask = message.params[2 + offset]
		let extended = message.params.count > 4 + offset
		let author = extended ? message.params[3 + offset].nicknameFromHostmask : nil
		...
		let took = output?.maskListEntryReceived(
			for: self,
			inChannelNamed: channelName,
			modeSymbol: hasModeLetterField ? message.params[2] : maskListModeSymbol(forNumeric: numeric),
			mask: mask,
			setBy: author,
			creationDate: date
		)
		...
		let text = ChannelMaskKind(numeric: numeric).entryText(
	...
	/** The mode letter the list a numeric belongs to is kept under.
	 ...
	 window. */
	private func maskListModeSymbol(forNumeric numeric: ServerNumeric) -> String {
		supportInfo.modeSymbol(forList: ChannelMaskKind(numeric: numeric).supportListKind) ?? ""
	}
}
