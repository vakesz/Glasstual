...
@MainActor
private func maximumHostmaskNicknameLength(on client: Client?) -> Int {
...
nonisolated extension NSString { // nonisolated: pure
	...
	var hostmask: Hostmask? {
		Hostmask(parsing: self as String, maximumNicknameLength: defaultHostmaskNicknameLength)
	}
	...
	var isHostmaskAddress: Bool {
		Hostmask.isValidAddress(self as String)
	}
	...
	var isHostmaskUsername: Bool {
		Hostmask.isValidUsername(self as String)
	}

	var isHostmaskNickname: Bool {
		Hostmask.isValidNickname(self as String, maximumLength: defaultHostmaskNicknameLength)
	}
...
extension NSString {
	...
	func hostmask(on client: Client?) -> Hostmask? {
		Hostmask(
	...
	func senderPrefix(on client: Client?) -> Prefix? {
		Prefix.user(
			parsing: self as String,
			maximumNicknameLength: maximumHostmaskNicknameLength(on: client)
		)
	...
	func isHostmaskNickname(on client: Client?) -> Bool {
	...
	func isChannelName(on client: Client) -> Bool {
		guard length > 0 else {
			return false
		}
