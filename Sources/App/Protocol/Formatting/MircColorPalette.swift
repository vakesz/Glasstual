...
// AppKit: a palette index names a colour the renderer and the editor both
// store as an `NSColor` attribute.
import AppKit
import CocoaExtensions

/** The colour an mIRC palette index names.

 The table is the protocol's, not any one view's: a `\u{3}` control code carries
 an index into it, so the transcript renderer and the input field's editor both
 resolve the same index the same way and both depend downward on this. */
nonisolated enum MircColorPalette {
	/// The colour `colorCode` names. The code is a palette index, which is what
	/// ``MircColor/palette(_:)`` and a formatting run's attribute both hold.
	static func mapColorCode(_ colorCode: UInt) -> NSColor { // nonisolated: pure
