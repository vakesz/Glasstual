...
/** Editing an attributed string's IRC formatting.
 ...
 The write half of the ``TextFormatterAttributeName`` attributes that
 ``TextFormatterEffects`` reads back when a line is cut for the wire: the
 formatting menu sets them on text a person is still typing and asks
 ``NSAttributedString/ircFormatterAttributeSet(inRange:range:)`` what is set.
 Both halves reach for `NSFont` and `NSColor`, so AppKit does not separate them
 either. */
extension NSMutableAttributedString {
	...
	private func addIRCColor(
		   ...
		   (0 ... TextFormatterColor.maximumPaletteIndex).contains(colorCode)
		{
			...
			addAttribute(appKitAttribute, value: MircColorPalette.mapColorCode(UInt(colorCode)), range: range)
	...
	func removeIRCFormatterAttribute(_ effect: TextFormatterEffectKind, range limitRange: NSRange) {
		enumerateAttributes(in: limitRange, options: .reverse) { attributes, effectiveRange, _ in
