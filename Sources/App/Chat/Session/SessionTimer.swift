...
/**
 Where a ``SessionTimer`` reads the time and where it waits.
...
final class SessionTimer {
	private let action: @MainActor (SessionTimer) -> Void
	...
	init(clock: TimerClock = .continuous, action: @escaping @MainActor (SessionTimer) -> Void) {
		self.action = action
		self.clock = clock
	...
	/// Starts the timer unless it is already running, so a repeated trigger
	/// keeps the deadline it already has rather than pushing it back.
	func startIfIdle(_ interval: TimeInterval, repeats: Bool = false, iterations: UInt = 0) {
		guard isActive == false else {
			return
		}

		start(interval, repeats: repeats, iterations: iterations)
	}

	/// A started timer that runs `action` once, `interval` seconds from now.
	///
	/// This is the replacement for an inline `Task { sleep; work }` deadline:
	/// the caller keeps the timer, so cancelling the pending work is `stop()`
	/// and asking whether it is still pending is ``isActive``.
	static func once(
		after interval: TimeInterval,
		clock: TimerClock = .continuous,
		_ action: @escaping @MainActor () -> Void
	) -> SessionTimer {
		let timer = SessionTimer(clock: clock) { _ in
			action()
		}
...
/**
 The session the timers built during `ServerSession.init` act on.

 Those timers are `let`s, so they are built before `super.init()` has run and
 before `self` can be captured by anything. Each action captures this box
 instead and resolves the session when it fires; the session fills the box in as
 soon as the superclass initialiser returns. The reference is weak, so a timer
 that somehow outlives its session fires into nothing rather than keeping it
 alive.
 */
@MainActor
final class SessionTimerOwner {
	weak var session: ServerSession?

	/// A timer whose action runs against the session this box resolves to.
	func makeTimer(_ action: @MainActor @escaping (ServerSession) -> Void) -> SessionTimer {
		SessionTimer { [self] _ in
