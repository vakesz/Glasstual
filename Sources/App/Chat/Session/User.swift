...
/** Someone visible to this session.

 A value. `id` is the person: it is minted once, when the session first sees the
 ...
 session's `UserPersistentStore` for that `id`.

 Editing means taking a copy, editing that, and handing it back to the directory
 (`ServerSession.modify(_:block:)`), which relinks the channels the person is in. */
nonisolated struct User: Identifiable, Hashable, Sendable, CustomStringConvertible {
	...
	/// The ban mask the setting asks for. The format is passed in because a
	/// user does not know its session; `ServerSession.banMask(for:)` reads it.
...
/** The state that belongs to a person rather than to one ``User`` value.
 ...
 session keeps one store per `User.ID` and hands it out by identity, which is
...
final class UserPersistentStore {
	...
	var relatedChannels: Set<Conversation> = []
	...
	/// Stops the removal timer. The session calls it when it drops the store;
