...
nonisolated enum MemberConversationDirection: Sendable {
...
/** A member of a channel.
 ...
 `prefixes` is the session's ISUPPORT `PREFIX` table as it stood when the list
 last stamped the member. Ranking and marks read it rather than the live session,
 because a member no longer knows one; `ConversationMembers.sortMembers()`
...
nonisolated struct Member: Identifiable, Hashable, Sendable {
	...
	/// Pure comparator. The setting is passed in so that a sort reads it once
	/// rather than once per comparison.
	func compare(usingWeights other: Member, favoringServerStaff favorIRCop: Bool) -> ComparisonResult {
	...
	/** Orders `members` by how much has been said to and by each of them.
	 ...
	 The one place a weight-ordered sort belongs, because a sort reads the
	 setting once: asking for it inside the comparator made a completion in a
	 two-thousand-member channel read `UserDefaults` some twenty-two thousand
	 times, once per comparison. The caller takes it from the connection's
	 setting snapshot, so a sort and the member list cannot disagree about it
	 mid-session. */
	static func sortedByConversationWeight(
		_ members: [Member],
		favoringServerStaff favorIRCop: Bool
	) -> [Member] {
		members.sorted { $0.compare(usingWeights: $1, favoringServerStaff: favorIRCop) == .orderedAscending }
	}

	func compareRank(to other: Member, favoringServerStaff favorIRCop: Bool) -> ComparisonResult {
		compareRank(to: other, favoringServerStaff: favorIRCop, casefoldingWith: prefixes)
	}

	/** Pure comparator. Both the setting and the table the nicknames fold
	 under are passed in, so that a sort reads the setting once and orders
