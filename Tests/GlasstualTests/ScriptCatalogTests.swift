...
struct ScriptCatalogTests {
	...
	func discoveryClassifiesAndResolvesCollisions() throws {
		...
		let catalog = ScriptCatalog.discover(customURL: custom, bundledURL: bundled, forbiddenCommands: ["join"])
		...
		#expect(UserScript(url: custom.appendingPathComponent("collision.sh"), origin: .custom)?
			.kind == .unixExecutable)
		#expect(ScriptCatalog.discover(
