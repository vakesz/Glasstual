...
@Suite("Typed settings store", .serialized)
@MainActor
struct TypedSettingsStoreTests {
	private static let boolKey = SettingsKey(
		"Tests -> Typed Store -> Flag",
		default: true,
		traits: [.unregistered, .uncatalogued]
	)

	private static let intKey = SettingsKey(
	...
	func unknownEnumerationCaseFallsBack() {
		let key = SettingsKeys.Input.tabKeyAction
		let original = key.storedValue
		defer { key.storedValue = original }
	...
	private static let declaredInt = SettingsKeys.Appearance.trackUserAwayStatusMaximumChannelSize
	private static let declaredBool = SettingsKeys.Messages.showJoinLeave

	@Test("Import coerces a number written as a string")
	func importCoercesStringsToNumbers() {
		#expect(
			SettingsKeys.coerce("1", forKey: Self.declaredInt.name) == .integer(1)
		)
	}

	/// A switch has to be stored as a boolean. The spellings `defaults write`
	/// accepts are not read, so a file that carries "yes" for a switch is a file
	/// that leaves the switch alone rather than one this build guesses at.
	@Test(
		"A boolean preference rejects the string spellings of true and false",
		arguments: ["1", "0", "true", "false", "yes", "no", "on", "off", "YES"]
	)
	func booleansRejectStringSpellings(spelling: String) {
		#expect(Bool.settingValue(from: spelling) == nil)
		#expect(SettingsKeys.coerce(.string(spelling), forKey: Self.declaredBool.name) == nil)
	}

	@Test("A boolean preference reads a stored boolean")
	func booleansReadStoredBooleans() {
		#expect(Bool.settingValue(from: NSNumber(value: true)) == true)
		#expect(Bool.settingValue(from: NSNumber(value: false)) == false)
		#expect(SettingsKeys.coerce(.boolean(true), forKey: Self.declaredBool.name) == .boolean(true))
	}

	@Test("Import rejects a value the declaration cannot represent")
	func importRejectsGarbage() {
		#expect(SettingsKeys.coerce("banana", forKey: Self.declaredInt.name) == nil)
		#expect(SettingsKeys.coerce(["a", "b"], forKey: Self.declaredBool.name) == nil)
	}
	...
	func integerConversionsAreExact() {
		for object: Any in [NSNumber(value: -1), NSNumber(value: 1.5), "-1", "1.5", "18446744073709551616"] {
			#expect(UInt.settingValue(from: object) == nil)
			#expect(UInt16.settingValue(from: object) == nil)
		}
		...
		{
			#expect(Int.settingValue(from: object) == nil)
		}
		#expect(UInt16.settingValue(from: 65536) == nil)
		#expect(UInt16.settingValue(from: "65536") == nil)
		#expect(SettingsKeys.coerce("1.5", forKey: Self.declaredInt.name) == nil)
	}

	@Test("Legacy integral numbers and numeric strings retain their exact values")
	func legacyNumbersRemainReadable() {
		#expect(Int.settingValue(from: NSNumber(value: Int.max)) == Int.max)
		#expect(Int.settingValue(from: String(Int.min)) == Int.min)
		#expect(UInt.settingValue(from: NSNumber(value: UInt.max)) == UInt.max)
		#expect(UInt.settingValue(from: String(UInt.max)) == UInt.max)
		#expect(UInt.settingValue(from: "18446744073709551615.0") == UInt.max)
		#expect(Int.settingValue(from: "9007199254740993.0") == 9_007_199_254_740_993)
		for object: Any in [NSNumber(value: 42.0), "42", "42.0", "4.2e1", "+42", "420e-1"] {
			#expect(Int.settingValue(from: object) == 42)
			#expect(UInt.settingValue(from: object) == 42)
			#expect(UInt16.settingValue(from: object) == 42)
		}
		#expect(UInt16.settingValue(from: "65535") == 65535)
		#expect(Double.settingValue(from: "1.25e2") == 125)
		#expect(UInt16.settingValue(from: "0xFF") == 255)
		#expect(Int.settingValue(from: "0x1.5p5") == 42)
		#expect(UInt.settingValue(from: "0x20000000000001") == 9_007_199_254_740_993)
		#expect(UInt.settingValue(from: "0xFFFFFFFFFFFFFFFF") == UInt.max)
		#expect(Int.settingValue(from: "0x1.0000000000000001") == nil)
	}
	...
	func invalidNumbersPreserveDefaults() {
		                    ...
		                    NSNumber(value: -Double.infinity), "nan", "inf", "-inf", "1e999"]
		{
			...
			#expect(UInt.settingValue(from: object) == nil)
			#expect(UInt16.settingValue(from: object) == nil)
			#expect(Double.settingValue(from: object) == nil)
		}
		withScratchKeys {
	...
	func nonFiniteDoubleIsNotStored() {
		let key = SettingsKey("Tests -> Typed Store -> Double", default: 2.5,
		                      traits: [.unregistered, .uncatalogued])
		defer { key.reset() }
		key.value = 4.5
	...
	func importPassesThroughUnknownKeys() {
		...
		let validated = SettingsKeys.coerce(payload, forKey: "Some Undeclared -> Key")
...
@Suite("Settings export contents", .serialized)
@MainActor
struct SettingsExportContentsTests {
	private var snapshot: SettingsArchive {
		SettingsArchive.snapshot(from: .live, sessions: [])
	}

	private func withRestored(_ key: SettingsKey<some Any>, _ body: () -> Void) {
		let original = key.storedValue
		defer { key.storedValue = original }
		body()
	}
	...
	func excludedKeysAreNotExported() {
		let excluded = SettingsKeys.Internals.runCount
		let suppression = SettingsKeys.Families.alertSuppression.pattern + "Tests Export"
		...
		defer { GlasstualUserDefaults.container.removeObject(forKey: suppression) }

		let exported = snapshot
		for name in [excluded.name, suppression] {
