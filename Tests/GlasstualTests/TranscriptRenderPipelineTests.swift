...
struct TranscriptRenderPipelineTests {
	...
	func barrierIncludesStandaloneWork() async {
		...
		let log = DeliveryLog()
		pipeline.submissions.yield(TranscriptRenderSubmission(isStandalone: true) {
			started.fire()
			await gate.wait()
	...
	func orderIsPreservedPerViewUnderABurst() async {
		let viewCount = 3
		let lineCount = 200
		let pipelines = (0 ..< viewCount).map { _ in TranscriptRenderPipeline() }
		...
		for line in 0 ..< lineCount {
			for (view, pipeline) in pipelines.enumerated() {
				...
				pipeline.submissions.yield(TranscriptRenderSubmission(isStandalone: false) {
					for _ in 0 ..< hops {
						await Task.yield()
					}
	...
	func barrierRejectsStoppedAndTerminatedPipelines() async {
		let stopped = TranscriptRenderPipeline()
		await stopped.stop()
		await stopped.barrier()

		let terminated = TranscriptRenderPipeline()
