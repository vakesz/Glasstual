...
private actor ScrollbackTestService {
	...
	func writeLogLine(_ logLine: ScrollbackEntry) -> ScrollbackWriteOutcome {
...
struct ScrollbackClientTests {
	...
	func cancelledFetchHasTypedOutcome() async {
		let gate = FetchGate()
		let queue = ScrollbackRequestQueue { _ in
			await gate.wait()
			return .page([])
		}
	...
	private nonisolated static func label(of request: ScrollbackFetchRequest) -> String { // nonisolated: pure
	...
	private static func enqueue(
		_ requests: [ScrollbackFetchRequest],
		on queue: ScrollbackRequestQueue
	) async -> [Task<[ScrollbackEntry], Never>] {
		var tasks: [Task<[ScrollbackEntry], Never>] = []
	...
	func fetchesAreServedInOrderPerView() async {
		let gate = FetchGate()
		let probe = IsolationProbe()
		let queue = ScrollbackRequestQueue { request in
			await gate.wait()
