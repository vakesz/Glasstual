...
struct MenuGraphTests {
	...
	func everyMenuTitleIsLocalized() throws {
		...
		let menus = [
			...
			controller.sidebarNoSelectionMenu,
		...
		let unlocalized = menus
			...
			/* Two items are deliberately untitled: the placeholders that give
			 Navigation ▸ Conversation List and Format submenus to hand to the
			 sidebar and to the window's formatter, both of which replace them wholesale
	...
	func sidebarTogglesLiveInTheViewMenu() throws {
		...
		for command in [MenuCommand.toggleSidebar, .toggleMemberList] {
		...
		let serverList = try #require(view.item(for: .toggleSidebar))
	...
	func editMenuLeavesTheSystemItemsToAppKit() throws {
		let controller = MenuActionController()
		let built = try #require(MenuGraph.builtMainMenu(for: controller).item(for: .editMenu)?.submenu)
	...
	func formatMenuCarriesTheStandardShortcuts() throws {
		...
		let formatter = TextFormattingMenu()
	...
	func channelAndQueryMenusStayInstalled() throws {
		...
		#expect(mainMenu.item(for: .queryMenu)?.submenu === controller.mainMenuDirectMenu)
	...
	func visibilityTogglesStartFromTheCatalog() throws {
		...
		#expect(
			mainMenu.item(for: .toggleSidebar)?.title
	...
	func menuBarShortcutsAreUnique() throws {
		let controller = MenuActionController()
		let formatMenu = try #require(TextFormattingMenu().makeMenu())
		let menus = [
			MenuGraph.builtMainMenu(for: controller),
			controller.mainMenuChannelMenu,
			controller.mainMenuDirectMenu,
			formatMenu,
		]
