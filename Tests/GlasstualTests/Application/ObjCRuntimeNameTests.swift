...
/// survive for an external reason: key-value observing, AppKit's saved
/// application state, and the mIRC palette, whose indices are wire format
...
struct ObjCRuntimeNameTests {
	...
	func observedKeyPathsResolve() {
		let observed: [(String, AnyKeyPath)] = [
			("ServerSession.isLoggedIn", \ServerSession.isLoggedIn),
		]
	...
	/// AppKit writes the restoration class's Objective-C name into the saved
	/// application state and looks the class up by that name on the next launch.
	/// Dropping the explicit `@objc` name would leave the mangled Swift name
	/// there, which changes with the module, and the window would silently stop
	/// restoring its full-screen state and its Space. Nothing in the compiler
	/// notices, so the name is pinned here.
	@Test("The main window is reachable by the name saved application state holds")
	func mainWindowRestorationNameResolves() {
		#expect(NSClassFromString("GlasstualMainWindow") === MainWindow.self)
	}
