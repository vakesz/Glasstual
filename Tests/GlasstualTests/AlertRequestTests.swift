...
/** Where an alert lands, given which windows are on screen.

 The quit confirmation asked for a sheet on the main window whether or not that
 window was on screen. Closed with Command-W, or with the application hidden,
 the sheet had nowhere to show, quitting waited on it forever, and a logout
 waited with it. */
@Suite("Alert host policy")
@MainActor
struct AlertHostPolicyTests {
	@Test(
		"A main-window alert runs modal while the main window is off screen",
		arguments: [false, true]
	)
	func mainWindowAlertNeedsAVisibleWindow(hasOtherVisibleWindow: Bool) {
		#expect(AlertHostPolicy.host(
			for: .mainWindow,
			sheetHostIsVisible: false,
			hasOtherVisibleWindow: hasOtherVisibleWindow
		) == .applicationModal)
		#expect(AlertHostPolicy.host(
			for: .mainWindow,
			sheetHostIsVisible: true,
			hasOtherVisibleWindow: hasOtherVisibleWindow
		) == .sheetHost)
	}

	@Test("Any visible window takes the alert when the main window cannot")
	func anyVisibleWindowFallsBackInOrder() {
		#expect(AlertHostPolicy.host(
			for: .anyVisibleWindow,
			sheetHostIsVisible: true,
			hasOtherVisibleWindow: true
		) == .sheetHost)
		#expect(AlertHostPolicy.host(
			for: .anyVisibleWindow,
			sheetHostIsVisible: false,
		...
		#expect(AlertHostPolicy.host(
			for: .anyVisibleWindow,
			sheetHostIsVisible: false,
			hasOtherVisibleWindow: false
		) == .applicationModal)
	}

	@Test("An application-modal alert stays modal with windows on screen")
	func applicationModalStaysModal() {
		#expect(AlertHostPolicy.host(
			for: .applicationModal,
			sheetHostIsVisible: true,
			hasOtherVisibleWindow: true
		) == .applicationModal)
	}
