...
@MainActor
struct TranscriptRendererSafetyTests {
	@Test(
	...
	func permittedSchemesRender(location: String) {
		#expect(TranscriptRenderer.isSafeLink(location))
	}
	...
	func refusedSchemesDoNotRender(location: String) {
		#expect(TranscriptRenderer.isSafeLink(location) == false)
	}
	...
	func zalgoTextIsStripped() {
		...
		let stripped = TranscriptRenderer.strippingDangerousUnicodeCharacters(zalgo)
	...
	func ordinaryAccentsSurvive() {
		...
		#expect(TranscriptRenderer.strippingDangerousUnicodeCharacters(accented) == accented)
	}
	...
	func zeroWidthHighlightsAreIgnored(pattern: String, source: String) {
		let body = TranscriptRenderer.renderNativeBody(source, withAttributes: TranscriptRenderOptions(
	...
	func nonemptyMatchAfterZeroWidthStillHighlights() {
		let body = TranscriptRenderer.renderNativeBody("ba", withAttributes: TranscriptRenderOptions(
			lineType: .privateMessage,
			highlightKeywords: ["a*"],
			textPolicy: TranscriptTextPolicy(highlightMatchingMethod: .regularExpression)
		), members: [])
