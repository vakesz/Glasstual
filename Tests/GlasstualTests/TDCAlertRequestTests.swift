...
private final class RecordingAlertPresenter: AlertPresenter {
	...
	init(response: TDCAlertResponse = .default, suppressionChecked: Bool = false) {
	...
	func present(_ request: AlertRequest, in presentation: AlertPresentation) async -> AlertPresenterResult {
		...
		case .sheet: presentations.append("sheet")
...
struct TDCAlertRequestTests {
	...
	func plainRequestIsPresented() async {
		...
		let outcome = await TDCAlert.run(request, on: .anyVisibleWindow, using: presenter)
	...
	func presentationIsForwarded() async {
		...
		await TDCAlert.run(request, on: .sheet(window), using: presenter)
		await TDCAlert.run(request, on: .applicationModal, using: presenter)
		TDCAlert.runModal(request, using: presenter)

		#expect(presenter.presentations == ["sheet", "modal", "runModal"])
	}
	...
	func suppressionKeyIsResolved() async {
		...
		await TDCAlert.run(request, on: .anyVisibleWindow, using: presenter)

		let presented = presenter.requests.first
		#expect(presented?.suppressionKey == TDCAlert.suppressionKey(withBase: baseKey))
	...
	func suppressionKeyIsNotDoublePrefixed() {
		let baseKey = Self.uniqueKey()
		let resolved = TDCAlert.suppressionKey(withBase: baseKey)
		#expect(TDCAlert.suppressionKey(withBase: resolved) == resolved)
	}
	...
	func suppressionIsRecordedAndHonoured() async {
		...
		let firstOutcome = await TDCAlert.run(request, on: .anyVisibleWindow, using: first)
		...
		let secondOutcome = await TDCAlert.run(request, on: .anyVisibleWindow, using: second)
	...
	func modalRunHonoursSuppression() {
		...
		_ = TDCAlert.runModal(request, using: first)
		#expect(first.requests.count == 1)

		let second = RecordingAlertPresenter(response: .alternate)
		let outcome = TDCAlert.runModal(request, using: second)
