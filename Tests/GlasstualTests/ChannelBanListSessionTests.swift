...
struct ChannelBanListSessionTests {
	private func entry(mask: String, author: String, created: Date?) -> ChannelBanListEntry {
	...
	func oversizedListLimitIsSaturated() throws {
		...
		let session = try #require(ChannelBanListSession(entryType: .ban, in: channel))
	...
	func withdrawnListModeLeavesNoSymbol() throws {
		...
		channel.activate()
		let session = try #require(ChannelBanListSession(entryType: .banException, in: channel))
