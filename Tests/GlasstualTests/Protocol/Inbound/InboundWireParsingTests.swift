...
/** Reading a line before anything has told the session what the server is like:
...
struct InboundWireParsingTests {
	...
	/** The inbound cap is deliberately looser than the fifteen parameters
	 RFC 1459 4.1 allows a command: what the session sends has to be a line every
	...
	/** A CTCP verb is an ASCII token and folds as one. Folding the whole
	 ...
	 the session then took seven characters off *unfolded* — and cost an
	...
	func banCarriesItsMaskBeforeISupport() throws {
		...
		#expect(session.supportInfo.configurationReceived == false)

		let modes = session.supportInfo.parseModes("+b nick!*@*")
	...
	func keyAndLimitCarryTheirParametersBeforeISupport() throws {
		...
		let message = try #require(
			Message(line: ":irc.example.net 324 me #chan +kl secret 50", on: session)
		)
	...
	func advertisedChannelModesWin() throws {
		let session = session()
		let message = try #require(
			Message(line: ":irc.example.net 005 me CHANMODES=b,k,l,imnpstL :are supported", on: session)
		)
		session.receiveNumericReply(message)

		let modes = session.supportInfo.parseModes("+L #other")
	...
	func memberComparisonFoldsBothSidesWithOneTable() {
		...
		let bracket = Member(user: User(nickname: "nick[home]"), prefixes: rfc1459)
		let brace = Member(user: User(nickname: "nick{home}"), prefixes: ascii)
	...
	/// The one weight-ordered sort, which reads the setting once for the
	...
	func conversationWeightOrdersTheHeaviestFirst() {
		...
		var heavy = Member(user: User(nickname: "zoe"), prefixes: table)
		heavy.incomingConversation()
		let light = Member(user: User(nickname: "alice"), prefixes: table)

		let sorted = Member.sortedByConversationWeight([light, heavy], favoringServerStaff: false)
