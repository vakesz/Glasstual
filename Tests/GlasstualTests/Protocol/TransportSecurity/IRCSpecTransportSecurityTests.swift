...
/// comma-separated key list, and what the session does with it depends on
...
struct IRCSpecTransportSecurityTests {
	...
	/// `sts`: "Sessions MUST ignore every key they do not understand", and a
	...
	/// A `duration` or `port` that is not a number is not a value the session
	...
	/// `sts`: "If the session is not already connected securely ... it MUST
	...
	/// `sts`: "Sessions MUST NOT save the policy if the connection's TLS
	...
	/// given, to the port the session is already on.
	...
	func validatedOffersStoreAPolicy() throws {
		...
		#expect(policyStore.enforcedPort(forHost: "irc.example.net") == 6697)
	}
	...
	func expiredPoliciesAreNotEnforced() {
		...
		#expect(policyStore.policy(forHost: "irc.example.net") == nil)
		#expect(policyStore.enforcedPort(forHost: "irc.example.net") == nil)
	}
}
