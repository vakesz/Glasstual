...
struct IRCSpecCasemappingTests {
	...
	func quitClosesTheFoldedQuery() throws {
		let session = session()
		let query = try #require(session.findConversationOrCreate("nick{home}", as: .direct))

		query.activate()

		try deliver(":nick[home]!u@example.org QUIT :Client Quit", on: session)
	...
	func nickRenamesTheFoldedQuery() throws {
		...
		try deliver(":nick[home]!u@example.org NICK :newnick", on: session)
	...
	func quitLeavesUnrelatedQueriesAlone() throws {
		let session = session()
		let query = try #require(session.findConversationOrCreate("someone", as: .direct))
