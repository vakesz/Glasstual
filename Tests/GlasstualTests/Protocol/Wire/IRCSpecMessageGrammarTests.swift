...
struct IRCSpecMessageGrammarTests {
	...
	func absentPrefixMeansTheServerSentIt() throws {
		...
		let message = try #require(Message(line: "PING :12345", on: session))
	...
	func fullHostmaskPrefixSplits() throws {
		let session = TestServerSession()
		let message = try #require(Message(line: ":nick!user@example.org PRIVMSG #chan :hi", on: session))
	...
	func serverNamePrefixIsAServer() throws {
		let session = TestServerSession()
		let message = try #require(Message(line: ":irc.example.org NOTICE * :hello", on: session))
	...
	func sessionOnlyTagsKeepTheirPlus() {
	...
	/// The escape table again, in the other direction: what the session writes
	...
	/// session does, and never the other way around.
