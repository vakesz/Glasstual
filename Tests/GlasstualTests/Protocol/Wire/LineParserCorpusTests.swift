...
/// Tag handling that depends on the capabilities a session negotiated.
@MainActor
struct MessageTagCorpusTests {
	@Test
	func messageWithoutSessionKeepsTagsButNoBatchOrTime() throws {
	...
	func serverTimeCapabilityDoesNotImplyPlayback() throws {
		...
		let message = try #require(
			Message(line: "@time=2021-01-01T00:00:00.000Z :nick!user@host PRIVMSG #chan :hi", on: session)
		)
	...
	func serverTimeAcceptsNumericTimestamps() throws {
		...
		let message = try #require(Message(line: "@t=1609459200 :nick!user@host PRIVMSG #chan :hi", on: session))
	...
	func unparsableServerTimeLeavesMessageCurrent() throws {
		...
		let message = try #require(Message(line: "@time=not-a-date :nick!user@host PRIVMSG #chan :hi", on: session))
