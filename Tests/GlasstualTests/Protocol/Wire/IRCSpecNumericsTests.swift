...
struct IRCSpecNumericsTests {
	/// RFC 2812 §5.1 and §5.2, plus the modern additions the session handles.
	nonisolated static let registry: [IRCSpecNumericCase] = [
		...
		IRCSpecNumericCase(name: "RPL_LUSERCLIENT", value: 251, numeric: .lusersession),
	...
	/// The session narrows the top of that band to 596 because 597 upwards is
	...
	func threeDigitCommandsAreNumerics() throws {
		...
		let numeric = try #require(Message(line: ":irc.example.net 433 * taken :Nickname is in use", on: session))
		let command = try #require(Message(line: ":irc.example.net NOTICE * :hello", on: session))
	...
	func welcomeCompletesRegistration() throws {
		...
		try receive(":irc.example.net 001 me_ :Welcome to the Internet Relay Network", on: session)

		#expect(session.isLoggedIn)
		#expect(session.userNickname == "me_")
	}

	/// modern.ircdocs.horse RPL_ISUPPORT: `<session> <token>[ <token>]{15} :are
	...
	func isupportFeedsOnlyTheTokens() throws {
		...
		#expect(session.supportInfo.maximumNicknameLength == 16)
		#expect(session.supportInfo.channelNamePrefixes == ["#", "&"])
		// "are" would otherwise be read as a token of its own.
		#expect(session.supportInfo.configurationReceived)
	}

	/// RFC 2812 §5.1 RPL_TOPIC: `<session> <channel> :<topic>`.
	...
	func topicNumericSetsTheTopic() throws {
		...
		try receive(":irc.example.net 332 me #chan :A topic worth reading", on: session)

		#expect(channel.topic == "A topic worth reading")
	}

	/// RFC 2812 §5.1 RPL_CHANNELMODEIS: `<session> <channel> <modestring>
	/// [modearguments...]`.
	@Test("324 records the channel's modes")
	func channelModeNumericRecordsTheModes() throws {
		let session = session()

		session.supportInfo.processConfigurationData("CHANMODES=b,k,l,imnpst")

		let channel = try #require(session.findConversationOrCreate("#chan"))

		channel.activate()

		try receive(":irc.example.net 324 me #chan +ntl 50", on: session)
	...
	func shortNumericsAreIgnored() throws {
		...
		try receive(":irc.example.net 332 me", on: session)
