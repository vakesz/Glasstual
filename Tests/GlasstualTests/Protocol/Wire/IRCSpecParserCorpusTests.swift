...
struct IRCSpecParserCorpusTests {
	...
	/// The corpus' candidate lines, rewritten the two ways a session's own line
	...
	/// The session's own encoder has to produce one of the encodings the corpus
	...
	/// Whatever encoding the session picks, reading it back has to recover the
	...
	func emptyTrailingParameterIsWrittenAsAColon() throws {
		#expect(try SendingMessage.string(wireCommand: "foo", arguments: ["bar", "baz", ""]) == "FOO bar baz :")
		#expect(try SendingMessage.string(command: .away, arguments: [""]) == "AWAY :")
		#expect(throws: SendingMessage.ArgumentError.self) {
			try SendingMessage.string(wireCommand: "foo", arguments: ["bar", "", "baz"])
		}
	}
	...
	func partialPrefixesOnTheWireNameAUser() throws {
		...
		let message = try #require(Message(line: ":coolguy@127.0.0.1 PRIVMSG #chan :hi", on: session))
	...
	func sourcesWithoutAHostStayServerNames(_ source: String) throws {
		let session = TestServerSession()
		let message = try #require(Message(line: ":\(source) NOTICE * :hello", on: session))
	...
	func bareNicknameOnTheWireNamesAUser() throws {
		let session = TestServerSession()
		let message = try #require(Message(line: ":coolguy NOTICE * :hello", on: session))
	...
	/// sessions should take whatever the server sends. Glasstual agrees: the
	...
	/// The half of the corpus a session does enforce: an address that is empty
