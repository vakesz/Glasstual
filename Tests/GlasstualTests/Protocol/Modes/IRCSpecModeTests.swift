...
struct IRCSpecModeTests {
	...
	/// bare `MODE #chan +b` is how a session asks for the ban list.
	...
	func wireLinesParseIntoTheSameChanges() throws {
		...
		let message = try #require(
			Message(line: ":op!u@h MODE #chan +oo-v alice bob carol", on: session)
		)
	...
	func trailingParameterModesParseTheSame() throws {
		let session = TestServerSession()
		let message = try #require(Message(line: ":op MODE #chan :+i", on: session))
