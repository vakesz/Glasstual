...
/** Registration against Libera.Chat, replayed line for line.
 ...
 first thing the session hears is a run of pre-registration notices, then the
 listing, then the acknowledgements. Captured 2026-09-09 with a plain probe. */
@MainActor
@Suite("Libera registration replay")
struct ServerSessionLiberaRegistrationReplayTests {
	...
	@Test("Without SASL the session ends negotiation after the delayed listing and logs in")
	func registersWithoutSASL() throws {
		...
		#expect(
			capabilityCommands(of: session).last == "END",
			"CAP END never went out: \(capabilityCommands(of: session))"
		)
		...
		#expect(session.isLoggedIn, "the session never treated 001 as a completed login")
	}

	@Test("With a password the session authenticates with SASL PLAIN, ends negotiation and logs in")
	func registersWithSASL() throws {
		...
		try replayRegistration(on: session) { session in
			...
			if sentLines(of: session)
				.contains(where: {
					$0.hasPrefix("AUTHENTICATE ") && $0 != "AUTHENTICATE PLAIN" && $0 != "AUTHENTICATE +"
				})
			{
				try receive(":iridium.libera.chat 900 me me!me@host me :You are now logged in as me", on: session)
				try receive(":iridium.libera.chat 903 me :SASL authentication successful", on: session)
			}
		}

		#expect(
			capabilityCommands(of: session).last == "END",
			"CAP END never went out: \(capabilityCommands(of: session)) sent=\(sentLines(of: session))"
		)
	...
	func batchedRequestIsATrailingParameterOnTheWire() throws {
		...
		let requests = sentLines(of: session).filter { $0.hasPrefix("CAP REQ") }
		...
		#expect(sentLines(of: session).last == "CAP END", "CAP END never went out: \(sentLines(of: session))")
	}

	// MARK: - Replay

	/// Feeds the notices and the listing, then answers every `CAP REQ` the
	/// session sends with the acknowledgement Libera would give, until the
	/// session stops asking.
	private func replayRegistration(
		on session: TestServerSession,
		afterEachAnswer: (TestServerSession) throws -> Void = { _ in }
	) throws {
		for line in Self.preRegistrationNotices {
			try receive(line, on: session)
		}
		try receive(Self.capabilityListing, on: session)

		var answered = 0
		for _ in 0 ..< 10 {
			let requests = capabilityCommands(of: session).filter { $0.hasPrefix("REQ ") }
			guard requests.count > answered else { break }
			for request in requests[answered...] {
				let names = request.dropFirst("REQ ".count)
				try receive(":iridium.libera.chat CAP me ACK :\(names)", on: session)
			}
			answered = requests.count
			try afterEachAnswer(session)
		}
	}
