...
@MainActor
struct ISupportCorpusTests {
	private static func supportInfo(_ configuration: String) -> ISupport {
	...
	func defaultsApplyBeforeAnyToken() {
		...
		#expect(supportInfo.maximumNicknameLength == UInt(ProtocolLimits.defaultNicknameMaximumLength))
	...
	func chunksTargetsByLimit() {
		...
		#expect(WireBatching.chunkTargets(targets, limit: 0) == [["a"], ["b"], ["c"], ["d"], ["e"]])
		#expect(WireBatching.chunkTargets([], limit: 3).isEmpty)
	}
	...
	func extractsStatusMessagePrefixes() {
		...
		#expect(supportInfo.extractStatusMessagePrefix(fromTargetNamed: "@#chan") == "@")
		#expect(supportInfo.extractStatusMessagePrefix(fromTargetNamed: "+&chan") == "+")
		#expect(supportInfo.extractStatusMessagePrefix(fromTargetNamed: "#chan") == "")
		#expect(supportInfo.extractStatusMessagePrefix(fromTargetNamed: "@") == "")
	}
	...
	/** `NSString.integerValue` used to parse these: it accepts trailing junk and
	 ...
	 the splitter would ever cut and the session stopped splitting at all. */
	@Test
	func numericTokensRejectTrailingJunkAndOverflow() {
		let junk = Self.supportInfo("NICKLEN=50abc")

		#expect(junk.maximumNicknameLength == UInt(ProtocolLimits.defaultNicknameMaximumLength))
	...
	func anOversizedLineLengthIsClamped() {
		...
		#expect(supportInfo.maximumLineLength == UInt(ProtocolLimits.maximumServerLineLength))
