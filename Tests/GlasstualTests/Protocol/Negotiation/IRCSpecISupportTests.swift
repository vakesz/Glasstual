...
struct IRCSpecISupportTests {
	...
	@Test("CASEMAPPING: the session compares its own nickname with the mapping")
	func sessionComparesNicknamesWithTheMapping() {
		let session = TestServerSession(configDictionary: ["nickname": "user[at]home"])

		session.supportInfo.processConfigurationData("CASEMAPPING=rfc1459")

		#expect(session.nicknameIsMyself("user{at}home"))
		#expect(session.nicknameIsMyself("USER{AT}HOME"))
		#expect(session.nicknameIsMyself("user(at)home") == false)
	}
	...
	func chanTypesListsChannelPrefixes() {
		...
		#expect(session.supportInfo.channelNamePrefixes == ["#", "&"])
		#expect(session.stringIsChannelName("#chan"))
		#expect(session.stringIsChannelName("&chan"))
		#expect(session.stringIsChannelName("+chan") == false)
		#expect(session.stringIsChannelName("nick") == false)
	}
	...
	func statusMessagePrefixIsRecognised() {
		...
		#expect(info.extractStatusMessagePrefix(fromTargetNamed: "@#chan") == "@")
		#expect(info.extractStatusMessagePrefix(fromTargetNamed: "+#chan") == "+")
		#expect(info.extractStatusMessagePrefix(fromTargetNamed: "#chan") == "")
		#expect(info.extractStatusMessagePrefix(fromTargetNamed: "%#chan") == "")
	...
	func statusMessageNeedsAChannelName() {
		...
		#expect(info.extractStatusMessagePrefix(fromTargetNamed: "@nick") == "")
		#expect(info.extractStatusMessagePrefix(fromTargetNamed: "@") == "")
	}

	// MARK: - TARGMAX, MAXTARGETS and MODES

	/** `TARGMAX=CMD:n[,CMD:n]`: a command with no number after the colon states
	 no limit, and the session stores that as the same zero it stores for a
	 command the token never named. Zero is therefore "the server said nothing
	 usable", not "unlimited": the session sends one target per line for it, the
	...
	func targetsAreChunkedToTheLimit() {
		...
		#expect(WireBatching.chunkTargets(targets, limit: 5) == [targets])
		#expect(WireBatching.chunkTargets(targets, limit: 0) == targets.map { [$0] })
	}
	...
	/// The `*LEN` tokens are byte budgets the session has to respect before it
	...
	func truncationCountsBytes() {
		#expect(ProtocolLimits.truncated("abcdef", toByteCount: 3) == "abc")
		#expect(ProtocolLimits.truncated("abc", toByteCount: 10) == "abc")
		// Each "é" is two bytes, so a four-byte budget holds exactly two.
		#expect(ProtocolLimits.truncated("ééé", toByteCount: 4) == "éé")
		#expect(ProtocolLimits.truncated("ééé", toByteCount: 5) == "éé")
		// Zero means the server advertised no limit.
		#expect(ProtocolLimits.truncated("abcdef", toByteCount: 0) == "abcdef")
	}
	...
	func nicknameLengthIsBounded() {
		...
		session.supportInfo.processConfigurationData("NICKLEN=8")
	...
	/// modern.ircdocs.horse: "The session MUST NOT assume that a parameter is
