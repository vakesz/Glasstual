...
struct SCRAMClientTests {
	...
	func clientFirstMessage() {
		...
		#expect(session.clientFirstMessage == "n,,n=user,r=rOprNGfwEbeRWgbNEkqO")
		#expect(session.state == .sentClientFirst)
	}
	...
	func clientFinalMessageMatchesRFC7677Vector() async throws {
		...
		let clientFinal = try await session.clientFinalMessage(forServerFirstMessage: serverFirst)
		let expected =
	...
	func verifyServerFinalMessageSucceedsForCorrectSignature() async throws {
		let session = exampleSession()
		_ = session.clientFirstMessage
		_ = try await session.clientFinalMessage(forServerFirstMessage: serverFirst)

		try session.verifyServerFinalMessage("v=6rriTRBi23WpRR/wtup+mMhUZUn/dB5nLTJRsjl95G4=")
	...
	func verifyServerFinalMessageRejectsWrongSignature() async throws {
		...
		let error = #expect(throws: (any Error).self) {
			try session.verifyServerFinalMessage("v=7rriTRBi23WpRR/wtup+mMhUZUn/dB5nLTJRsjl95G4=")
		}
	...
	private func expectFailure(
		...
		let error = await #expect(throws: (any Error).self, sourceLocation: sourceLocation) {
			try await session.clientFinalMessage(forServerFirstMessage: serverMessage)
		}
