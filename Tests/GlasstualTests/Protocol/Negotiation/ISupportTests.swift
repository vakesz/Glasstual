...
struct ISupportTests {
	...
	func lengthBudgetsReachCommandConsumers(_ limit: UInt) {
		...
		let awaySession = fixture.chatSession.createSession(with: ServerConfig())
		awaySession.isLoggedIn = true
		awaySession.supportInfo.processConfigurationData("AWAYLEN=\(limit)")
		awaySession.sendCommand("AWAY \u{e9}\u{e9}ab", completeTarget: false, target: nil)
		...
		session.sendCommand("TOPIC #test \u{e9}\u{e9}ab", completeTarget: false, target: nil)
	...
	/// NICKLEN is what the server lets this session register, not a promise
	...
	func nicknameLimitsReachNameConsumers(_ limit: UInt) throws {
		let session = TestServerSession()
		session.supportInfo.processConfigurationData("NICKLEN=\(limit)")
		#expect(session.stringIsNickname("alice"))
		#expect("alice!u@host".hostmask(on: session)?.nickname == "alice")
		let message = try #require(Message(line: ":alice!u@host PRIVMSG #test :hello", on: session))
		#expect(message.senderIsServer == false)
		#expect(message.senderNickname == "alice")
		#expect("alice".padNickname(
			withCharacter: 95,
			maximumLength: session.supportInfo.maximumNicknameLength
		)
			== (limit == 3 ? "al_" : "alice_"))
	}
	...
	func widerNicknameLimitWidensParsing(_ advertisesWiderLimit: Bool) throws {
		...
		if advertisesWiderLimit {
			session.supportInfo.processConfigurationData("NICKLEN=64")
		}
		let nickname = String(repeating: "n", count: 60)
		let message = try #require(Message(line: ":\(nickname)!u@host PRIVMSG #test :hello", on: session))
	...
	func targetCountsReachTheChunkingConsumer(_ limit: UInt) {
		...
		#expect(WireBatching.chunkTargets([], limit: info.maximumTargets(forCommand: "PRIVMSG")).isEmpty)
	}
	...
	func limitTokensAndTargetChunking() {
		...
		#expect(WireBatching.chunkTargets(targets, limit: 2) == [["a", "b"], ["c", "d"], ["e"]])

		let conservativeChunks = WireBatching.chunkTargets(["a", "b"], limit: 0)
	...
	/// names it will hold. Only the first half was read, so the session kept
	...
	/** `ServerSession.supportInfo` is `lazy`, so the table is built the first time
	 anything reads it — and that can be long after the session recorded the
	 capability facts `WATCH` and `MONITOR` stand in for. Construction used to
	 run the reconnect reset and call straight back into the session, so the first
	 read of the table turned `WATCH` back off: `enableCapability(.watchCommand)`
	 reads the ceiling on its way to asking about the tracked peers and undid
	 itself, and nothing was ever sent. A reset now only reports what it cleared,
	 so nothing the session did not ask for can withdraw a fact. */
	@Test("Building the table withdraws no capability the session already recorded")
	func aFreshTableWithdrawsNothing() {
		...
		#expect(session.isCapabilityEnabled(.watchCommand))
		/* The first read of the lazy table, which is what used to withdraw it. */
		#expect(session.supportInfo.maximumWatchEntries == 0)
		#expect(session.isCapabilityEnabled(.watchCommand))
	}

	/// A reconnect really has lost what the old server advertised, so the reset
	/// that follows one reports the facts those tokens stood in for and the
	/// session withdraws them.
	@Test("Resetting the table after a reconnect withdraws them")
	func aResetWithdrawsTheFacts() {
		let session = TestServerSession()
		session.markAsLoggedIn()
		session.apply(session.supportInfo.processConfigurationData("WATCH=128"))

		#expect(session.isCapabilityEnabled(.watchCommand))

		session.apply(session.supportInfo.reset())

		#expect(session.isCapabilityEnabled(.watchCommand) == false)
		#expect(session.supportInfo.maximumWatchEntries == 0)
	}

	/// The pre-CAP `PROTOCTL` spellings go out once, and the fact the session
	/// records from the first line is what stops the next one repeating them.
	@Test("NAMESX and UHNAMES write their PROTOCTL line once")
	func legacyPrefixTokensWriteTheirLineOnce() {
		let session = TestServerSession()
		session.markAsLoggedIn()

		session.apply(session.supportInfo.processConfigurationData("NAMESX UHNAMES"))
		session.apply(session.supportInfo.processConfigurationData("NAMESX UHNAMES"))

		let sentLines = (session.sentLines as NSArray).compactMap { $0 as? String }
		#expect(sentLines == ["PROTOCTL NAMESX", "PROTOCTL UHNAMES"])
		#expect(session.isCapabilityEnabled([.multiPrefix, .userhostInNames]))
	}
