...
struct IRCSpecCapabilityBehaviourTests {
	...
	private func printedChannelNames(of session: TestServerSession) -> [String] {
		session.printedLines.compactMap { ($0 as? [String: Any])?["channel"] as? Conversation }.map(\.name)
	}

	// MARK: - server-time

	/// `server-time`: `<value> ::= YYYY-MM-DDThh:mm:ss.sssZ`. A message
	/// carrying one is replayed — it happened when the tag says, not when it
	/// arrived.
	...
	func serverTimeSetsTheMessageTime() throws {
		...
		let message = try #require(
			Message(line: "@time=2011-10-19T16:40:51.620Z :nick!u@h PRIVMSG #chan :hi", on: session)
		)
	...
	/// A `time` tag the session cannot read must leave the arrival time alone
	...
	func unreadableServerTimeIsIgnored(_ value: String) throws {
		...
		let message = try #require(
			Message(line: "@time=\(value) :nick!u@h PRIVMSG #chan :hi", on: session)
		)
	...
	func wholeSecondTimestampsAreRead() throws {
		...
		let message = try #require(
			Message(line: "@time=2011-10-19T16:40:51Z :nick!u@h PRIVMSG #chan :hi", on: session)
		)
	...
	/// which the session still reads for their sake.
	...
	func unixTimestampTagIsRead() throws {
		...
		let message = try #require(
			Message(line: "@t=1319042451.620 :nick!u@h PRIVMSG #chan :hi", on: session)
		)
	...
	func echoedPrivateMessageLandsInTheTargetWindow() throws {
		...
		let query = try #require(session.findConversationOrCreate("bob", as: .direct))

		query.activate()

		try deliver(":me!u@h PRIVMSG bob :hello there", on: session)

		#expect(printedChannelNames(of: session) == ["bob"])
	}
	...
	func echoedCTCPQueryIsNotAnswered() throws {
		...
		try deliver(":me!u@h PRIVMSG bob :\u{01}VERSION\u{01}", on: session)
	...
	func awayNotifyTogglesTheAwayFlag() throws {
		...
		try deliver(":alice!ali@example.org JOIN #chan", on: session)
		try deliver(":alice!ali@example.org AWAY :out to lunch", on: session)

		#expect(try #require(channel.findMember("alice")).user.isAway)

		try deliver(":alice!ali@example.org AWAY", on: session)

		#expect(try #require(channel.findMember("alice")).user.isAway == false)
	}

	/// Without the capability the session has no reason to believe an `AWAY`
	...
	func accountNotifySetsAndClears() throws {
		...
		try deliver(":alice!ali@example.org JOIN #chan", on: session)
		try deliver(":alice!ali@example.org ACCOUNT aliceacct", on: session)

		#expect(try #require(channel.findMember("alice")).user.account == "aliceacct")

		try deliver(":alice!ali@example.org ACCOUNT *", on: session)
	...
	func accountTagIdentifiesTheSender() throws {
		...
		try deliver(":alice!ali@example.org JOIN #chan", on: session)
		try deliver("@account=aliceacct :alice!ali@example.org PRIVMSG #chan :hi", on: session)
	...
	func changeHostReplacesUserAndHost() throws {
		...
		session.enableCapability(.changeHost)

		let channel = try joinedChannel("#chan", on: session)

		try deliver(":alice!ali@example.org JOIN #chan", on: session)
		try deliver(":alice!ali@example.org CHGHOST newuser cloak.example.net", on: session)
	...
	func malformedChangeHostIsRejected() throws {
		...
		try deliver(":alice!ali@example.org JOIN #chan", on: session)
		try deliver(":alice!ali@example.org CHGHOST new user cloak.example.net", on: session)
	...
	func setNameUpdatesTheRealName() throws {
		...
		try deliver(":alice!ali@example.org JOIN #chan", on: session)
		try deliver(":alice!ali@example.org SETNAME :Alice of Example", on: session)
	...
	func identityMessagesNeedTheirCapability() throws {
		...
		try deliver(":alice!ali@example.org ACCOUNT aliceacct", on: session)
		try deliver(":alice!ali@example.org CHGHOST newuser cloak.example.net", on: session)
		try deliver(":alice!ali@example.org SETNAME :Someone Else", on: session)
	...
	func standardRepliesAreRecognised(_ command: String) throws {
		let session = session()
		let before = session.printedLines.count
		...
		#expect(session.printedLines.count > before)
	}
	...
	func standardRepliesRouteToTheirChannel() throws {
		...
		#expect(printedChannelNames(of: session) == ["#chan"])
	}
	...
	func shortStandardRepliesAreIgnored() throws {
		...
		try deliver(":irc.example.net FAIL :something went wrong", on: session)
