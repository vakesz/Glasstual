...
struct MircColorFormatTests {
	...
	func booleanEffectsUseMatchingOpenAndCloseCharacters() throws {
		...
		#expect(bold.controlCharacter == TextFormatterControlCharacter.bold)
		#expect(italic.controlCharacter == TextFormatterControlCharacter.italic)
		#expect(monospace.controlCharacter == TextFormatterControlCharacter.monospace)
		#expect(strike.controlCharacter == TextFormatterControlCharacter.strikethrough)
		#expect(underline.controlCharacter == TextFormatterControlCharacter.underline)
		...
		#expect(controlCharacter(at: 0, in: buffer) == TextFormatterControlCharacter.bold)
		#expect(controlCharacter(at: 2, in: buffer) == TextFormatterControlCharacter.bold)
	}
	...
	func digitAndHexColorsEncodeValuesAndBackgroundNeedsMatchingForeground() throws {
		...
		#expect(digitForeground.controlCharacter == TextFormatterControlCharacter.colorDigit)
		#expect(digitForeground.value == "04")
		#expect(digitForeground.length == 4)
		#expect(hexForeground.controlCharacter == TextFormatterControlCharacter.colorHex)
	...
	func stringFormattedForIRCWrapsSegmentsWithControlCharacters() {
		...
		#expect(controlCharacter(at: 0, in: formatted) == TextFormatterControlCharacter.bold)
		#expect(controlCharacter(at: 1, in: formatted) == TextFormatterControlCharacter.italic)
		#expect((formatted as NSString).substring(with: NSRange(location: 2, length: 5)) == "hello")
		#expect(controlCharacter(at: 7, in: formatted) == TextFormatterControlCharacter.italic)
		#expect(controlCharacter(at: 8, in: formatted) == TextFormatterControlCharacter.bold)
	}
	...
	func linesConsumeEveryCharacterExactlyOnce() {
		...
		var cursor = LineCursor(NSAttributedString(string: payload))
		...
		while true {
			...
			guard cursor.nextLine(forTarget: "#test", on: session, with: .notice) != nil else {
				break
			}
	...
	func formattedLinesReassembleIntoTheSource() {
		...
		let bold = formatterKey(TextFormatterAttributeName.boldAttributeName)
		let foreground = formatterKey(TextFormatterAttributeName.foregroundColorAttributeName)
		...
		while true {
			let before = cursor.length
			guard let line = cursor.nextLine(forTarget: "#test", on: session, with: .privateMessage) else {
				break
			}
	...
	func background99ClearsTheBackground() {
		let components = ("\u{3}04,99" as NSString).colorComponents(
			ofCharacter: TextFormatterControlCharacter.colorDigit,
			startingAt: 0
		)
	...
	func bareColourCodesClearBothHalves() {
		...
		let shortHex = ("\u{4}0F" as NSString).colorComponents(
			ofCharacter: TextFormatterControlCharacter.colorHex,
			startingAt: 0
		)
	...
	func hexadecimalCodeReadsBothHalves() {
		...
		#expect(components.foreground == .color(.rgb(MircColorChannels(
			red: 1, green: Double(0x80) / 0xFF, blue: 0
		))))
		#expect(components.background == .color(.rgb(MircColorChannels(
	...
	private func endsMidColourCode(_ line: String) -> Bool {
		...
		guard let index = scalars.lastIndex(where: { $0.value == UInt32(TextFormatterControlCharacter.colorDigit) })
		else {
			return false
		}
	...
	func bothEntryPointsRemoveCompleteControlSequences() {
	...
	func malformedColorSeparatorsAndUnicodeArePreserved() {
	...
	private func stripControlCharacters(from line: String) -> String {
		...
		while let scalar = scalars.popFirst() {
			switch scalar.value {
			case UInt32(TextFormatterControlCharacter.bold), UInt32(TextFormatterControlCharacter.terminator):
				continue
			case UInt32(TextFormatterControlCharacter.colorDigit):
