...
struct NicknameColorsTests {
	...
	func hashRemainsCompatibleWithLegacyMD5ByteOrder() {
		let hash = NicknameColors.hash(for: "alice")
	...
	func colorIsStable() {
		let first = NicknameColors.color(for: "alice")
		let second = NicknameColors.color(for: "Alice")

		#expect(first.hexadecimalString == second.hexadecimalString)
	}
	...
	func colorsContrastWithDefaultGrounds() {
		...
		for isDark in [false, true] {
			...
			for name in names {
				let color = NicknameColors.color(for: name, isDark: isDark)
	...
	func appearancesDifferAndOverridesWin() {
		...
		let dark = NicknameColors.color(for: nickname, isDark: true)
		let light = NicknameColors.color(for: nickname, isDark: false)
