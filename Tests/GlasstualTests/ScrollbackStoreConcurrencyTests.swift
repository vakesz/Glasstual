...
private actor ScrollbackStoreHarness {
	...
	private let filenameStore: ScrollbackFilenameFixture
	...
	func seed(_ rows: [(identifier: UInt, entry: ScrollbackEntry)]) async throws {
		try await ScrollbackFixture.withContext(at: directory.appendingPathComponent(filenameStore.filename)) { context in
			for row in rows {
				ScrollbackDatabase.insert(row.entry, in: context, entryIdentifier: row.identifier)
			}
			try context.save()
		}
	}
	...
	func persistedRows(inView view: String) async throws -> [(identifier: UInt, entry: ScrollbackEntry)] {
		try await ScrollbackFixture.withContext(at: directory.appendingPathComponent(filenameStore.filename)) { context in
			...
			request.predicate = NSPredicate(
				format: "%K == %@", ScrollbackAttribute.logLineViewIdentifier.rawValue, view
			)
			request.sortDescriptors = [
				NSSortDescriptor(key: ScrollbackAttribute.entryIdentifier.rawValue, ascending: true),
				NSSortDescriptor(key: ScrollbackAttribute.logLineUniqueIdentifier.rawValue, ascending: true),
			]
			return try context.fetch(request).map { object in
				let identifier = try #require(object
					.value(forKey: ScrollbackAttribute.entryIdentifier.rawValue) as? NSNumber)
	...
	func write(_ index: Int, inView view: String) async {
		await store.writeLogLine(ScrollbackEntry(
...
struct ScrollbackStoreConcurrencyTests {
	...
	func failedOpenPreservesTheSelectedDatabase(inferable: Bool) async throws {
		...
		let entityName = inferable ? "UnrecognizedHistoryVersion" : ScrollbackDatabase.entityName
		let original = try await context.perform {
			let attribute = NSAttributeDescription()
			attribute.name = ScrollbackAttribute.entryIdentifier.rawValue
			...
			object.setValue(payload, forKey: ScrollbackAttribute.entryIdentifier.rawValue)
			try context.save()
		...
		let archives = try await context.perform {
			...
			let archives = try context.fetch(request).compactMap {
				$0.value(forKey: ScrollbackAttribute.entryIdentifier.rawValue) as? Data
			}
			context.reset()
			return archives
		}
