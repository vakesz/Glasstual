...
struct ConnectionInboundDeliveryTests {
	...
	func unavailableConnectionService() async throws {
		...
		let connection = Connection(config: ConnectionConfig(), onClient: client, closeClock: .continuous,
	...
	func finalErrorBeforeEOF() async throws {
		...
		try await observe(.ClientDidDisconnect, from: client) {
	...
	func stsPreservesEndpointPassword(_ storedInKeychain: Bool) async throws {
		...
		client.socket = plain
		try await observe(.ClientDidConnect, from: client) {
	...
	private func observe(
		_ name: Notification.Name, from client: Client, performing action: () -> Void
	) async throws {
	...
	func closeDeadlineCompletesExactlyOnce() async {
		...
		let connection = Connection(config: ConnectionConfig(), onClient: client, closeClock: clock) {
