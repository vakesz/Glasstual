...
struct MessageRuleEngineTests {
	...
	func remoteSeparatorsCannotStartACommand(_ separator: (name: String, scalar: String)) {
		let commands = MessageRuleEngine.actionCommands(
	...
	func unknownTokenExpandsToNothing() {
		#expect(
			MessageRuleEngine.actionCommands(in: "/say [%_channelName_%]", replacing: ["%_channelName_%": ""])
				== ["say []"]
		)
	}
