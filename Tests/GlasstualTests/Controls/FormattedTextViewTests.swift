...
@Suite("Formatted text view values")
@MainActor
struct FormattedTextViewTests {
	/// Exercise editing with the same window and first-responder ownership as
	/// the input bar. Its undo stack belongs to the editor.
	private func makeField() -> (window: NSWindow, field: FormattedTextView) {
		let window = NSWindow(
	...
	private func typeAnUndoableEdit(into field: FormattedTextView) throws {
	...
	func replacingTheValueKeepsOtherUndoActions() throws {
		let (window, field) = makeField()
		defer { window.close() }
		let other = FormattedTextView(frame: NSRect(x: 0, y: 40, width: 320, height: 40))
...
private final class TypingRecordingField: FormattedTextView {
