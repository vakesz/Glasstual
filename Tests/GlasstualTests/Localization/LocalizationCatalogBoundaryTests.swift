...
struct LocalizationCatalogBoundaryTests {
	...
	func channelMaskListCopyAndFormatting() {
		#expect(ChannelMaskKind.ban.heading(channelName: "#swift") == "Bans in #swift")
		#expect(
			ChannelMaskKind.banException.heading(channelName: "#swift") == "Ban Exceptions in #swift"
		)
		#expect(
			ChannelMaskKind.inviteException.heading(channelName: "#swift")
				== "Invite Exceptions in #swift"
		)
		#expect(ChannelMaskKind.quiet.heading(channelName: "#swift") == "Quiets in #swift")
		#expect(ChannelMaskListModel.entryCountDescription(4, maximum: 0, isTruncated: false) == "4 entries")
		#expect(ChannelMaskListModel.entryCountDescription(4, maximum: 100, isTruncated: false) == "4 of 100 entries")
		/* A list cut at the window's cap must not read as a complete one that
		 happens to be exactly that long. */
		#expect(ChannelMaskListModel.entryCountDescription(4, maximum: 100, isTruncated: true) == "First 4 entries")
		/* The notice under the list says why the count is not the whole of it;
		 repeating the count in it said the same thing twice. */
		#expect(
			String(localized: .ChannelProperties.listTruncatedNotice)
	...
	func accessListCountsFormatOneNumberOneWay() {
		...
		#expect(ChannelMaskListModel.entryCountDescription(20000, maximum: 0, isTruncated: false).contains(formatted))
		#expect(ChannelMaskListModel.entryCountDescription(20000, maximum: 50000, isTruncated: false).contains(formatted))
		#expect(ChannelMaskListModel.entryCountDescription(20000, maximum: 50000, isTruncated: true).contains(formatted))
	}
