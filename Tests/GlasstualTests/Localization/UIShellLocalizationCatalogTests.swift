...
struct UIShellLocalizationCatalogTests {
	...
	func onboardingBoundaryResolvesSemanticCopyAndFormatting() {
		...
		#expect(String(localized: .NetworkPicker.customServer) == "Custom Server")
	...
	func serverPropertiesBoundaryUsesDomainTypesAndPreservesMultilineCopy() {
		#expect(String(localized: AddressBookEntryKind.ignore.listTitle) == "User Ignore")
		#expect(String(localized: AddressBookEntryKind.userTracking.listTitle) == "User Tracking")
	...
	func mainWindowBoundaryUsesTypedStatusAndRankMappings() {
		...
		#expect(MenuCommand.sidebarTitle(isVisible: false) == "Show Server List")
		#expect(MenuCommand.sidebarTitle(isVisible: true) == "Hide Server List")
	...
	private func catalog(named tableName: String) throws -> UIShellCatalog {
		let sourcesURL = RepositoryPaths.appSources
