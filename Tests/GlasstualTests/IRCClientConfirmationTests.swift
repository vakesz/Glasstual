...
struct IRCClientConfirmationTests {
	...
	@Test("Cancelling a confirmation dismisses its wait without a reply")
	func cancellationEndsTheWait() async throws {
		...
		client.requestConfirmation(AlertRequest(
			title: "Confirm",
			body: "Pending operation",
			defaultButton: "Continue",
			alternateButton: "Cancel"
		)) { _ in
			Issue.record("Cancelled operation executed")
		}
		let pending = try #require(client.pendingConfirmationTasks.values.first)
