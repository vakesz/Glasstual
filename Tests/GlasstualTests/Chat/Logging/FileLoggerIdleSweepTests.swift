...
struct FileLoggerIdleSweepTests {
	...
	func sessionBannersSurviveLoggerRelease() async throws {
		...
		settings.logToDiskIsEnabled = true
		...
		for body in ["first body", "second body"] {
			session.transcriptLog.logger = FileLogger(session: session, commands: commands, destination: destination)
			var line = ChatLine()
			line.messageBody = body
			session.writeToLogFile(line)
			#expect(session.logFileSessionIsOpen)
			session.closeLogFile()
			#expect(!session.logFileSessionIsOpen)
			#expect(session.transcriptLog.logger == nil)
		}
		let drained = await withCheckedContinuation { continuation in
			commands.finish { continuation.resume(returning: $0) }
		}
