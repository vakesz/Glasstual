...
struct NetworkCatalogTransportTests {
	...
	/// Probed against each network's advertised TLS endpoint. These three are the
	...
	@Test(
		...
		arguments: ["2600net", "GeekShed", "Slashnet"]
	)
	...
	@Test("IRCnet uses its documented plaintext endpoint while TLS fails hostname verification")
	func ircnetUsesDocumentedEndpoint() throws {
		let network = try #require(NetworkList().network(named: "IRCnet"))
		#expect(network.serverAddress == "irc.ircnet.com")
		#expect(network.serverPort == 6667)
		#expect(network.prefersSecuredConnection == false)
	}
	...
	private static func networksCatalogKeys() throws -> Set<String> {
		let url = RepositoryPaths.appSources.appending(path: "Chat/NetworkCatalog/Networks.xcstrings")
