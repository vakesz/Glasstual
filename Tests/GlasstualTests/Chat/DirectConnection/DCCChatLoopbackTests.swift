...
struct DCCChatLoopbackTests {
	...
	func cancellingASessionEndsItsEventsWithoutAVerdict() async throws {
		let listening = ChatFixture.listeningConnection()
		var dialled: DirectChatSocket?
		var collected: Task<[DirectChatEvent], Never>?
	...
	/** Binding the port and waiting for the peer share one window, the same way
	 a file transfer's do, so a chat nobody dials into gives the port back
	 instead of holding a router mapping for as long as the app runs. */
	@Test("A listening chat with no peer expires", .timeLimit(.minutes(1)))
	func acceptanceDeadline() async {
		let listening = DirectChatSocket(config: DirectChatSocket.Config(
			endpoint: .listen(portRange: ChatFixture.portRange),
			sendTimeout: .seconds(20),
			acceptanceTimeout: .milliseconds(100)
		))
		...
		#expect(events.last.flatMap(ChatFixture.failure) == .connectTimeout)
...
enum ChatFixture {
	...
	static func listeningConnection(maximumLineLength: Int = 16 * 1024) -> DirectChatSocket {
		DirectChatSocket(config: DirectChatSocket.Config(
	...
	/// The error a `closed` event carried, and `nil` for any other event.
	static func failure(_ event: DirectChatEvent) -> DCCTransferError? {
		guard case let .closed(error) = event else {
			return nil
		}
	...
	static func allEvents(of connection: DirectChatSocket) -> Task<[DirectChatEvent], Never> {
		let events = connection.events

		return Task {
			var collected: [DirectChatEvent] = []
