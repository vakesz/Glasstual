...
struct DCCTransferLoopbackTests {
	...
	func emptyFileCompletes(senderListens: Bool) async throws {
		...
		let listener = DCCTransfer(config: senderListens
		...
		for await event in listener.events {
			...
			if case let .listening(port) = event {
				let peer = DCCTransfer(config: senderListens
	...
	func fiveMegabyteFileSurvivesTheRoundTrip() async throws {
		...
		for await event in sender.events {
			...
			let receiver = try DCCTransfer(config: TransferFixture.diallingReceiver(
				port: port,
	...
	func cancellingATransferEndsItsEventsWithoutAVerdict() async throws {
		...
		let receiver = try DCCTransfer(config: TransferFixture.listeningReceiver(
			file: TransferFixture.destination(destination),
			fileSize: UInt64(payload.count)
		))
		...
		for await event in receiver.events {
			...
			case let .listening(port):
				let sender = try DCCTransfer(config: TransferFixture.diallingSender(
					port: port,
					file: TransferFixture.source(source),
					fileSize: UInt64(payload.count)
				))
	...
	func resumeListeningSender() async throws {
		...
		for await event in sender.events {
			...
			if case let .listening(port) = event {
				...
				var config = TransferFixture.diallingReceiver(
					port: port,
					file: file,
					fileSize: UInt64(payload.count)
				)
	...
	func acceptanceDeadline() async throws {
		...
		var config = try TransferFixture.listeningSender(file: TransferFixture.source(source), fileSize: 1)
		config.acceptanceTimeout = .milliseconds(100)
		let sender = DCCTransfer(config: config)
	...
	func receiverInactivityDeadline() async throws {
		let directory = try TransferFixture.makeDirectory()
		defer { TransferFixture.remove(directory) }
		var config = try TransferFixture.listeningReceiver(
			file: TransferFixture.destination(directory.appendingPathComponent("destination")),
			fileSize: 42
		)
		config.inactivityTimeout = .milliseconds(100)
		let receiver = DCCTransfer(config: config)
...
enum TransferFixture {
	...
	static func listeningSender(file: DCCTransferFile, fileSize: UInt64) -> DCCTransfer.Config {
		DCCTransfer.Config(
			role: .sender,
	...
	static func listeningReceiver(file: DCCTransferFile, fileSize: UInt64) -> DCCTransfer.Config {
		DCCTransfer.Config(
			role: .receiver,
			endpoint: .listen(portRange: portRange),
			file: file,
