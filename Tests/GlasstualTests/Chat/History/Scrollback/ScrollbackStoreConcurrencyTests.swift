...
private actor ScrollbackStoreHarness {
	private let store: ScrollbackStore
	private let recorder: DeletionRecorder
	private let filenameSetting: ScrollbackFilenameFixture

	let directory: URL

	init() throws {
		directory = URL(fileURLWithPath: NSTemporaryDirectory())
			.appendingPathComponent("scrollback-log-\(UUID().uuidString)", isDirectory: true)
		...
		store = ScrollbackStore(filenameSetting: filenameSetting.store, deletionHandler: { identifiers, _ in
	...
	func seed(_ rows: [(identifier: UInt, entry: ScrollbackEntry)]) async throws {
		try await ScrollbackFixture.withContext(at: directory.appendingPathComponent(filenameSetting.filename)) { context in
			for row in rows {
				ScrollbackQueries.insert(row.entry, in: context, entryIdentifier: row.identifier)
			}
			try context.save()
		}
	}

	func persistedRows(inView view: String) async throws -> [(identifier: UInt, entry: ScrollbackEntry)] {
		try await ScrollbackFixture.withContext(at: directory.appendingPathComponent(filenameSetting.filename)) { context in
			let request = NSFetchRequest<NSManagedObject>(entityName: ScrollbackQueries.entityName)
			request.predicate = NSPredicate(
				format: "%K == %@", ScrollbackAttribute.viewID.rawValue, view
			)
			request.sortDescriptors = [
				NSSortDescriptor(key: ScrollbackAttribute.entryID.rawValue, ascending: true),
				NSSortDescriptor(key: ScrollbackAttribute.lineID.rawValue, ascending: true),
			]
			return try context.fetch(request).map { object in
				let identifier = try #require(object
					.value(forKey: ScrollbackAttribute.entryID.rawValue) as? NSNumber)
				let entry = try #require(ScrollbackEntry(managedObject: object))
				return (identifier.uintValue, entry)
			}
		}
	}

	func write(_ entry: ScrollbackEntry) async {
		await store.writeChatLine(entry)
	}

	/// A page named by the line it ends before, oldest first, or — with no
	/// anchor — the newest rows the view holds, newest first.
	func fetch(
		inView view: String,
		before: String? = nil,
		limit: UInt = 100,
		limitToDate: Date? = nil
	) async -> [ScrollbackEntry] {
		if let before {
			return await store.fetchOutcome(ScrollbackFetchRequest(
				viewIdentifier: view,
		...
		return await store.fetchOutcome(ScrollbackFetchRequest(
			viewIdentifier: view,
			kind: .rowPage(before: nil, fetchLimit: limit, limitToDate: limitToDate)
		)).entries
	}
	...
	func write(_ index: Int, inView view: String) async {
		await store.writeChatLine(ScrollbackEntry(
			lineData: Data("line \(index)".utf8),
...
@Suite("Scrollback store", .serialized)
struct ScrollbackStoreConcurrencyTests {
	...
	func typedStoreFetchDistinguishesOutcomes() async throws {
		let harness = try ScrollbackStoreHarness()
		let newest = ScrollbackFetchRequest(viewIdentifier: "history",
		...
		let older = ScrollbackFetchRequest(
	...
	private func entry(_ label: String, date: TimeInterval, view: String = "history") -> ScrollbackEntry {
		var line = ChatLine()
		line.receivedAt = Date(timeIntervalSince1970: date)
		line.messageBody = label
		return line.scrollbackEntry(forView: view)
	}
	...
	func chronologicalKeysetPagesVisitEachRowOnce(persisted: Bool) async throws {
		...
		let newestFirst = await harness.fetch(inView: "history", limit: 100)
		#expect(newestFirst.map(\.uniqueIdentifier) == Array(expected.reversed()))
		var page = await Array(harness.fetch(inView: "history", limit: 2).reversed())
	...
	func duplicateInsertionIdentifiersRemainUnchangedAcrossReopens() async throws {
		...
		for _ in 0 ..< 2 {
			#expect(await harness.openDatabase())
			var page = await harness.fetch(inView: "history", limit: 1)
			var visited = page.map(\.uniqueIdentifier)
			for _ in 0 ..< entries.count {
				guard let anchor = page.first?.uniqueIdentifier else { break }
	...
	func failedOpenPreservesTheSelectedDatabase(inferable: Bool) async throws {
		...
		let payload = Data("irreplaceable scrollback archive".utf8)
		let entityName = inferable ? "UnrecognizedHistoryVersion" : ScrollbackQueries.entityName
		let original = try await context.perform {
			let attribute = NSAttributeDescription()
			attribute.name = ScrollbackAttribute.entryID.rawValue
			...
			object.setValue(payload, forKey: ScrollbackAttribute.entryID.rawValue)
			try context.save()
		...
		let archives = try await context.perform {
			...
			let archives = try context.fetch(request).compactMap {
				$0.value(forKey: ScrollbackAttribute.entryID.rawValue) as? Data
			}
			context.reset()
			return archives
		}
		...
		#expect(databases.map(\.lastPathComponent) == [filenameSetting.filename])
		await harness.shutdown()
	}
