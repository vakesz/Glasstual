...
/// session list, so an existing settings file survives the move to Codable.
@Suite("Server endpoint property-list round trip")
struct ServerEndpointCodableTests {
	...
	func roundTripsAStoredDictionary() throws {
		// Captured from the class-based `ServerEndpoint.dictionaryValue`.
		...
		let server = try #require(PropertyListModel.decode(ServerEndpoint.self, from: fixture))
	...
	func passwordIsNeverEncoded() {
		var server = ServerEndpoint(serverAddress: "irc.example.test")
