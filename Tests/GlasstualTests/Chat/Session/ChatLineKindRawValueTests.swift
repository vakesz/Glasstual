...
/// every stored entry, so the numbers are pinned here as well as in source.
@Suite("Log line enumeration raw values")
@MainActor
struct ChatLineKindRawValueTests {
	@Test("Line type numbers are stable", arguments: [
		(ChatLineKind.undefined, UInt(0)),
		(.action, 1),
		(.actionNoHighlight, 2),
		(.ctcp, 3),
		(.ctcpQuery, 4),
	...
	func lineTypeRawValuesAreStable(type: ChatLineKind, rawValue: UInt) {
		#expect(type.rawValue == rawValue)
		#expect(ChatLineKind(rawValue: rawValue) == type)
	}

	@Test("Member type numbers are stable")
	func memberTypeRawValuesAreStable() {
		#expect(ChatLineMemberKind.normal.rawValue == 0)
		#expect(ChatLineMemberKind.localUser.rawValue == 1)
	}

	@Test("Delivery state numbers are stable")
	func deliveryStateRawValuesAreStable() {
		#expect(ChatLineDeliveryState.none.rawValue == 0)
		#expect(ChatLineDeliveryState.pending.rawValue == 1)
		#expect(ChatLineDeliveryState.delivered.rawValue == 2)
		#expect(ChatLineDeliveryState.failed.rawValue == 3)
	}
	...
	func numbersAreContiguous() {
		let highest = ChatLineKind.website.rawValue
		let resolved = (0 ... highest).compactMap { ChatLineKind(rawValue: $0) }
