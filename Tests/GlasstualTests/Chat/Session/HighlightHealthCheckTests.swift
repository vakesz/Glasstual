...
struct HighlightHealthCheckTests {
	...
	func malformedHighlightEntriesAreSkippedOnLoad() {
		let config = PropertyListModel.decode(ServerConfig.self, from: [
			"highlightList": [
				["matchKeyword": "keep"],
...
@MainActor
struct UserSessionReferenceTests {
	/** A user used to hold a weak session and a member reached its own session
	 through it, so both read live state that could already have gone. Neither
	 holds a session now: a user is a value, and a member carries the prefix table
	 the list stamped it with. */
	@Test
	func aUserAndItsMemberOutliveTheirSession() throws {
		var session: TestServerSession? = TestServerSession()
		let user = User(nickname: "nick")
		var member = try Member(user: user, prefixes: #require(session).currentUserPrefixes)
