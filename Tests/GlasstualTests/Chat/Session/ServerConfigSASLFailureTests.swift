...
/// the session completes registration unauthenticated on 904, 905 or 906, which
/// many networks treat as a security failure.
@Suite("Disconnect on SASL failure")
@MainActor
struct ServerConfigSASLFailureTests {
	private static let key = "disconnectOnSASLFailure"

	@Test("The option is off by default")
	func defaultsToOff() {
		#expect(ServerConfig().disconnectOnSASLFailure == false)
	}
	...
	func absentKeyReadsAsOff() throws {
		var dictionary = ServerConfig().dictionaryValue
	...
	func offIsTheDefault() {
		#expect(ServerConfig().dictionaryValue[Self.key] == nil)
	}
