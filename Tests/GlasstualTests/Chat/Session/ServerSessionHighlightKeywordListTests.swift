...
/// keyword lists the session attached to them.
@MainActor
private final class RecordingConversationPresentation: ChatItemPresenting {
	...
	func print(_ chatLine: ChatLine, completionBlock _: PrintedLineCompletion?) {
...
/** The two lists a printed line is matched against are assembled per line, out
 ...
 are actually used, rather than off the assembler; the session's own print
 observer is cleared so that the production path runs to the end. */
@MainActor
@Suite("Highlight keyword lists")
struct ServerSessionHighlightKeywordListTests {
	private func session(
		...
		settings.highlightMatchKeywords = matchKeywords
		settings.highlightExcludeKeywords = excludeKeywords
		settings.highlightCurrentNickname = highlightCurrentNickname
		settings.highlightMatchingMethod = matchingMethod
	...
	private func channel(
		...
		drawnInto presentation: RecordingConversationPresentation
	) throws -> Conversation {
	...
	private func keywordLists(
		fromRemoteMessageIn channel: Conversation,
		on session: TestServerSession,
		drawnInto presentation: RecordingConversationPresentation
	...
	func globalListsAreCarried() throws {
		let session = session(matchKeywords: ["alpha"], excludeKeywords: ["beta"])
		let presentation = RecordingConversationPresentation()
		let channel = try channel(named: "#chat", on: session, drawnInto: presentation)

		let lists = try keywordLists(fromRemoteMessageIn: channel, on: session, drawnInto: presentation)
	...
	func disallowedLinesGetNoLists() throws {
		...
		let ignored = try keywordLists(fromRemoteMessageIn: channel, on: session, drawnInto: presentation)
	...
	func localNicknameIsAppendedExceptForRegularExpressions() throws {
		let partial = session(matchKeywords: ["alpha"], highlightCurrentNickname: true)
		let partialPresentation = RecordingConversationPresentation()
		...
		let regularPresentation = RecordingConversationPresentation()
	...
	func localNicknameIsAppendedOnlyOnce() throws {
		let session = session(matchKeywords: ["mara"], highlightCurrentNickname: true)
		let presentation = RecordingConversationPresentation()
		let channel = try channel(named: "#chat", on: session, drawnInto: presentation)

		#expect(
			try keywordLists(fromRemoteMessageIn: channel, on: session, drawnInto: presentation)
				.match == ["mara"]
		)
	}
	...
	func channelScopedRulesApplyOnlyToTheirChannel() throws {
		let session = session()
		let chatPresentation = RecordingConversationPresentation()
		let otherPresentation = RecordingConversationPresentation()
		let chat = try channel(named: "#chat", on: session, drawnInto: chatPresentation)
		let other = try channel(named: "#other", on: session, drawnInto: otherPresentation)
		...
		#expect(
			try keywordLists(fromRemoteMessageIn: chat, on: session, drawnInto: chatPresentation)
				.match == ["everywhere", "chat-only"]
		)
		#expect(
			try keywordLists(fromRemoteMessageIn: other, on: session, drawnInto: otherPresentation)
