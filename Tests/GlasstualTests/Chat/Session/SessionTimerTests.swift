...
private final class TestClock {
	...
	/// The clock to hand a `SessionTimer`.
...
struct SessionTimerTests {
	...
	func startsInactive() {
		let clock = TestClock()
		let timer = SessionTimer(clock: clock.timerClock) { _ in }
	...
	func cancellationPreventsTheAction() async {
		let clock = TestClock()
		var fired = 0
		let timer = SessionTimer(clock: clock.timerClock) { _ in fired += 1 }
	...
	func theActionCanStopItsOwnTimer() async {
		let clock = TestClock()
		var fired = 0
		let timer = SessionTimer(clock: clock.timerClock) { timer in
			fired += 1
			timer.stop()
		}
	...
	/// a disconnected session, a closed channel — is not there to be called back.
