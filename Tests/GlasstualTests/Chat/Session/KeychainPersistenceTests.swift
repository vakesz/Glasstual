...
struct KeychainPersistenceTests {
	...
	func settingsSaveIncludesPreparationAndCommit() async {
		...
		let pendingSaves = saves.waitForSaves()
		let quit = Task {
	...
	func settingsSaveFailurePreventsTermination() async {
		...
		let quit = saves.waitForSaves()
		release.finish()
		#expect(await !quit.value)
		await save.value
		#expect(await saves.waitForSaves().value)
	}
	...
	func settingsSaveAcceptedDuringWait() async {
		...
		let first = saves.submit {
			for await _ in firstGate {}
			return true
		}
		let quit = saves.waitForSaves()
		let second = saves.submit {
	...
	private func credentialRefreshKeepsEndpoint(request: EndpointRequest, acknowledgePassword: Bool) async throws {
		...
		let origin = ServerEndpoint(serverAddress: "origin.invalid")
		var config = ServerConfig()
		config.serverList = [origin, ServerEndpoint(serverAddress: "fallback.invalid")]
		let session = ServerSession(config: config, environment: fixture.environment)
		session.config.pendingNicknamePassword = .set("pending")
		...
		session.pendingEndpoint = explicit
		var printed: [String] = []
		session.linePrintObserver = { printed.append($0.messageBody) }
		let (firstGate, releaseFirst) = AsyncStream<Void>.makeStream()
		let (secondGate, releaseSecond) = AsyncStream<Void>.makeStream()
		...
		session.credentialLoader = { await loader.load($0) }
		session.connect()
		var iterator = started.makeAsyncIterator()
		#expect(await iterator.next() == 1)
		let selectedIndex = session.lastServerSelected
		if acknowledgePassword {
			let edits = session.config.pendingKeychainEdits
			session.config.acknowledgeKeychainEdits(edits)
		} else {
			session.config.connectionName = "Renamed during credential lookup"
		}
		releaseFirst.finish()
		#expect(await iterator.next() == 2)
		...
		#expect(session.lastServerSelected == selectedIndex)
	...
	func configuredRetryUsesLatestEndpoint() throws {
		let fixture = ChatEnvironmentFixture()
		var original = ServerEndpoint(serverAddress: "original.invalid")
		let fallback = ServerEndpoint(serverAddress: "fallback.invalid")
		var config = ServerConfig()
		config.serverList = [original, fallback]
		let session = ServerSession(config: config, environment: fixture.environment)
		#expect(try #require(session.takeConnectionEndpoint()).serverAddress == original.serverAddress)
		original.serverAddress = "edited.invalid"
		original.serverPort = 7001
		original.prefersSecuredConnection = true
		session.config.serverList = [fallback, original]
		let retry = try #require(session.takeConnectionEndpoint(retryingServerIdentifier: original.uniqueIdentifier))
		#expect(retry.serverAddress == "edited.invalid")
		#expect(retry.serverPort == 7001)
		#expect(retry.connectionPrefersSecuredConnection)
		#expect(session.lastServerSelected == 1)
		#expect(try #require(session.takeConnectionEndpoint()).serverAddress == fallback.serverAddress)
	}
	...
	func configurationChangeDuringCredentialLoad() async throws {
		...
		var iterator = started.makeAsyncIterator()
		#expect(await iterator.next() == 1)
		var replacement = ServerEndpoint()
		replacement.serverAddress = "new.example"
		session.config.serverList = [replacement]
		releaseFirst.finish()
		#expect(await iterator.next() == 2)
		#expect(session.socket == nil)
