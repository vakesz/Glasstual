...
struct TranscriptControllerHistoryLatchTests {
	...
	func reloadWithoutAnItemDoesNotLatchTheFlag() async throws {
		var client: Client? = Client(config: ClientConfig())
		let window = window()
		let controller = try TranscriptController(client: #require(client), in: window)
		controller.loadsHistoryLazily = { false }
		defer { controller.tearDown(.permanentRemoval) }
	...
	func serverHistoryRetryAvailability() {
		...
		let console = TranscriptController(client: client, in: window)
