...
struct ChannelTopicFeatureTests {
	...
	func contentUsesNamespacedLocalizedCopy() {
		#expect(String(localized: .ChannelTopic.topicLabel("#swift")) == "Topic for #swift")
		#expect(
			String(localized: .ChannelTopic.editorAccessibilityHint)
				== "Edit the topic. IRC text formatting is preserved."
		)
		#expect(String(localized: .ChannelTopic.changeTopicButton) == "Change Topic")
		#expect(String(localized: .ChannelTopic.cancelButton) == "Cancel")
	}
	...
	func remainingLengthCountsDownAndGatesSubmission() {
		...
		#expect(ChannelTopicModel.lengthFooter(remaining: 1) == "1 byte remaining")

		model.formattedTopic = "123456"
		#expect(model.remainingLength == -1)
		#expect(model.fitsMaximumLength == false)
		#expect(ChannelTopicModel.lengthFooter(remaining: -1) == "1 byte too many")
		#expect(ChannelTopicModel.lengthFooter(remaining: 2) == "2 bytes remaining")
	...
	func returnRespectsTopicLengthLimit() {
		...
		let coordinator = ChannelTopicEditor(
			...
			accessibilityLabel: String(localized: .ChannelTopic.topicLabel(channel.name)),
