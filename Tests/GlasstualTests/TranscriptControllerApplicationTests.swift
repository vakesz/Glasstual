...
struct TranscriptControllerApplicationTests {
	...
	/** The controller refers to its window weakly, as it does to its client, so
	...
	func staleOlderPageCannotMutateController(destroy: Bool) async throws {
		...
		#expect(try Scrollback.shared.containsMessageIdentifier(
	...
	func retiredServerPageCannotMutateController() async throws {
		try await withServerHistoryController(labeled: true) { client, channel, controller in
			...
			#expect(!Scrollback.shared.containsMessageIdentifier(
				identifier,
	...
	func duplicateOnlyServerPageDoesNotExhaust() async throws {
		try await withServerHistoryController(labeled: true) { client, channel, controller in
			...
			Scrollback.shared.indexLogLines([duplicate], forView: channel.uniqueIdentifier)
			for wire in [
	...
	func nestedServerPagePrepends() async throws {
		try await withServerHistoryController(labeled: true) { client, channel, controller in
			...
			var outcome = await ScrollbackClient.shared.fetchOutcome(fetch)
			while case .failed(.missingCursor) = outcome, clock.now < deadline {
				try await Task.sleep(for: .milliseconds(1))
				outcome = await ScrollbackClient.shared.fetchOutcome(fetch)
			}
	...
	private func withServerHistoryController(
		labeled: Bool,
		_ body: (TestClient, Channel, TranscriptController) async throws -> Void
	) async throws {
