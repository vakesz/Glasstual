...
struct ScrollbackRestampTests {
	...
	private func rowWrittenByAnEarlierVersion(
		...
		said: Date,
		stored: Date
	) -> (entry: ScrollbackEntry, receivedAt: Date) {
		var line = LogLine()
	...
	private func seed(_ rows: [ScrollbackEntry], at url: URL, startingAt firstIdentifier: UInt) async throws {
		try await ScrollbackFixture.withContext(at: url) { context in
			for (offset, row) in rows.enumerated() {
				ScrollbackDatabase.insert(
					row,
					in: context,
	...
	private func restampIsOutstanding(at url: URL) async throws -> Bool {
		try await ScrollbackFixture.withContext(at: url) { context in
			ScrollbackDatabase.needsEntryCreationDateRestamp(in: context)
		}
	}
