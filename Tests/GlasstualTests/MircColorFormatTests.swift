...
struct MircColorFormatTests {
	...
	func booleanEffectsUseMatchingOpenAndCloseCharacters() throws {
		...
		#expect(bold.controlCharacter == unichar(TextFormatterControlCharacter.bold))
		#expect(italic.controlCharacter == unichar(TextFormatterControlCharacter.italic))
		#expect(monospace.controlCharacter == unichar(TextFormatterControlCharacter.monospace))
		#expect(strike.controlCharacter == unichar(TextFormatterControlCharacter.strikethrough))
		#expect(underline.controlCharacter == unichar(TextFormatterControlCharacter.underline))
		...
		#expect(controlCharacter(at: 0, in: buffer) == unichar(TextFormatterControlCharacter.bold))
		#expect(controlCharacter(at: 2, in: buffer) == unichar(TextFormatterControlCharacter.bold))
	}
	...
	func digitAndHexColorsEncodeValuesAndBackgroundNeedsMatchingForeground() throws {
		...
		#expect(digitForeground.controlCharacter == unichar(TextFormatterControlCharacter.colorDigit))
		#expect(digitForeground.value == "04")
		#expect(digitForeground.length == 4)
		#expect(hexForeground.controlCharacter == unichar(TextFormatterControlCharacter.colorHex))
	...
	func stringFormattedForIRCWrapsSegmentsWithControlCharacters() {
		...
		#expect(controlCharacter(at: 0, in: formatted) == unichar(TextFormatterControlCharacter.bold))
		#expect(controlCharacter(at: 1, in: formatted) == unichar(TextFormatterControlCharacter.italic))
		#expect((formatted as NSString).substring(with: NSRange(location: 2, length: 5)) == "hello")
		#expect(controlCharacter(at: 7, in: formatted) == unichar(TextFormatterControlCharacter.italic))
		#expect(controlCharacter(at: 8, in: formatted) == unichar(TextFormatterControlCharacter.bold))
	}
	...
	func background99ClearsTheBackground() {
		let components = ("\u{3}04,99" as NSString).colorComponents(
			ofCharacter: unichar(TextFormatterControlCharacter.colorDigit),
			startingAt: 0
		)
	...
	func bareColourCodesClearBothHalves() {
		...
		let shortHex = ("\u{4}0F" as NSString).colorComponents(
			ofCharacter: unichar(TextFormatterControlCharacter.colorHex),
			startingAt: 0
		)
	...
	private func endsMidColourCode(_ line: String) -> Bool {
		...
		guard let index = scalars.lastIndex(where: { Int($0.value) == TextFormatterControlCharacter.colorDigit })
		else {
			return false
		}
	...
	private func stripControlCharacters(from line: String) -> String {
		...
		while let scalar = scalars.popFirst() {
			...
			case TextFormatterControlCharacter.bold, TextFormatterControlCharacter.terminator:
				continue
