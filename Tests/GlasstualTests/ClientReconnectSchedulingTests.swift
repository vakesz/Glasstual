...
/** The ping and pong schedule is covered by `ClientConnectionTimerPolicy`;
...
struct ClientReconnectSchedulingTests {
	...
	func theDelayBacksOff() {
		let delay = { (attempt: UInt) in
			ClientConnectionTimerPolicy.reconnectDelay(attempt: attempt, jitter: 0)
		}

		#expect(delay(0) == ClientConnectionTimerPolicy.reconnectInterval)
		#expect(delay(1) == 40)
		#expect(delay(2) == 80)
		#expect(delay(3) == 160)
		#expect(delay(4) == ClientConnectionTimerPolicy.maximumReconnectInterval)
		#expect(delay(50) == ClientConnectionTimerPolicy.maximumReconnectInterval)
	}

	@Test("Jitter shortens the delay by at most a quarter, and never past zero")
	func jitterIsBounded() {
		for attempt in UInt(0) ... 8 {
			let full = ClientConnectionTimerPolicy.reconnectDelay(attempt: attempt, jitter: 0)

			for jitter in [0.0, 0.25, 0.5, 1.0, -3.0, 7.0] {
				let delay = ClientConnectionTimerPolicy.reconnectDelay(attempt: attempt, jitter: jitter)
