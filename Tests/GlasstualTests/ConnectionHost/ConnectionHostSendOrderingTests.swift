...
/// A plain TCP listener on loopback that keeps everything the session wrote, in
...
private final class SendOrderingSessionShim: NSObject, RemoteConnectionClientProtocol {
	...
	func didSecureConnection(withProtocolType _: tls_protocol_version_t,
...
nonisolated struct ConnectionHostSendOrderingTests {
	...
	func connectCommandsPrecedeJoinOnWire() async throws {
		...
		let session = TestServerSession(configDictionary: [
			"nickname": "tester",
			"loginCommands": ["raw MODE tester +i", "msg NickServ IDENTIFY test-only", "raw WHOIS tester"],
		])
		session.forwardsSentLines = true
		session.forwardsProcessedMessages = true
		_ = try #require(session.findConversationOrCreate("#order"))
		var config = ConnectionConfig()
		config.serverAddress = "127.0.0.1"
		config.serverPort = port
		config.diagnostics = ConnectionDiagnostics()
		let connection = Connection(config: config, onSession: session)
	...
	func stalledHandshakeDoesNotBlockControl() async throws {
		...
		service.exportedObject = SendOrderingSessionShim(events: continuation)
		service.resume()
		let deadline = Task {
	...
	func readsWaitForTheApplicationsAcknowledgement() async throws {
		...
		service.remoteObjectInterface = RemoteConnectionInterface.server()
		service.exportedInterface = RemoteConnectionInterface.client()
		service.exportedObject = SendOrderingSessionShim(events: continuation, withholdsAcknowledgements: true)
		service.resume()
		let deadline = Task {
	...
	private static func driveConnection(
		...
		config.setFloodControl(delayInterval: 60, maximumMessages: 1)

		let (events, continuation) = AsyncStream<ConnectionEvent>.makeStream()
		let shim = SendOrderingSessionShim(events: continuation)
