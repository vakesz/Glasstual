...
struct ScrollbackTransactionTests {
	...
	func failedCloseRearmsPeriodicSaves() async throws {
		...
		try await context.perform {
			ScrollbackDatabase.insert(row, in: context, entryIdentifier: 1)
			let inserted = try #require(context.insertedObjects.first)
			inserted.setValue(nil, forKey: ScrollbackAttribute.sessionIdentifier.rawValue)
		}
	...
	func failedTransactionsRetainState() async throws {
		...
		try await context.perform {
			let inserted = try #require(context.insertedObjects.first)
			#expect(inserted.value(forKey: ScrollbackAttribute.logLineData.rawValue) as? Data == row.data)
			inserted.setValue(
				NSNumber(value: row.sessionIdentifier),
				forKey: ScrollbackAttribute.sessionIdentifier.rawValue
			)
