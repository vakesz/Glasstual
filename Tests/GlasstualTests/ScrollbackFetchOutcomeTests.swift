...
struct ScrollbackFetchOutcomeTests {
	...
	func databaseReadOutcome(failing: Bool) async throws {
		...
		try await context.perform {
			...
			let result = ScrollbackDatabase.fetchOutcome(
			...
			if failing {
				...
				let older = ScrollbackDatabase.fetchOutcome(
				...
				#expect(ScrollbackDatabase.fetchOutcome(
					in: context, viewIdentifier: "view", ascending: true, fetchLimit: 10, limitToDate: nil
				).entries.isEmpty)
				return
			}
			...
			let entry = ScrollbackEntry(logLineData: Data(), uniqueIdentifier: "anchor", viewIdentifier: "view",
			                            sessionIdentifier: 1, creationDate: 1)
			ScrollbackDatabase.insert(entry, in: context, entryIdentifier: 1)
			let oldest = ScrollbackDatabase.fetchOutcome(in: context, viewIdentifier: "view", before: "anchor",
			...
			let missing = ScrollbackDatabase.fetchOutcome(in: context, viewIdentifier: "view", before: "missing",
			                                              fetchLimit: 10, limitToDate: nil)
			guard case .failed(.missingCursor) = missing else {
				Issue.record("A missing cursor was classified as exhaustion")
				return
			}
			ScrollbackDatabase.insert(entry, in: context, entryIdentifier: 2)
			let duplicate = ScrollbackDatabase.fetchOutcome(in: context, viewIdentifier: "view", before: "anchor",
			                                                fetchLimit: 10, limitToDate: nil)
			guard case .failed(.ambiguousCursor) = duplicate else {
				Issue.record("An ambiguous cursor was classified as exhaustion")
				return
			}
			let invalid = ScrollbackEntry(logLineData: Data(), uniqueIdentifier: "invalid", viewIdentifier: "view",
			                              sessionIdentifier: 1, creationDate: -1)
			ScrollbackDatabase.insert(invalid, in: context, entryIdentifier: 3)
			let corrupt = ScrollbackDatabase.fetchOutcome(in: context, viewIdentifier: "view", ascending: true,
			                                              fetchLimit: 10, limitToDate: nil)
			...
			#expect(try context
				.count(for: NSFetchRequest<NSManagedObject>(entityName: ScrollbackDatabase.entityName)) == 3)
		}
	}
}
