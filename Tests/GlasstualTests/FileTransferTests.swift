...
struct FileTransferTests {
	...
	func senderFollowsNicknameDuringReservation() async throws {
		...
		let preparation = Task {
			await FileTransfer.sender(for: client, nickname: "alice", path: source.path) { _, _, _ in
				didStart.yield(())
				for await _ in gate {}
				return file
			}
	...
	func emptyFileFactories() async throws {
		...
		let client = TestClient()
		let outgoing = try #require(await FileTransfer.sender(for: client, nickname: "alice", path: source.path))
		let incoming = try #require(FileTransfer.receiver(for: client, nickname: "alice", address: "127.0.0.1",
	...
	func activeSenderCommitsOffset() async throws {
		...
		try payload.write(to: source)
		let client = TestClient()
		let sender = try #require(await FileTransfer.sender(for: client, nickname: "alice", path: source.path))
	...
	func recipientsOwnIndependentSources() async throws {
		...
		let client = TestClient()
		let first = try #require(await FileTransfer.sender(for: client, nickname: "alice", path: source.path))
		let second = try #require(await FileTransfer.sender(for: client, nickname: "bob", path: source.path))
	...
	func acklessRowStatus() throws {
		...
		let unacknowledged = String(localized: .FileTransfers.sentToWithoutAPeerAcknowledgement("alice"))
	...
	private func assertOfferTargets(_ nickname: String, from controller: FileTransfer, on client: TestClient) {
	...
	private func receiver(on client: Client, port: UInt16 = 1234, size: UInt64 = 2048,
	                      token: String? = nil) throws -> FileTransfer
	{
