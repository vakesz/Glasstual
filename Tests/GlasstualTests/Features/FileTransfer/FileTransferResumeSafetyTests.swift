...
struct FileTransferResumeSafetyTests {
	...
	func anExistingFileIsNotResumedInto() async throws {
		...
		/* Nothing to resume from, so the session offers no RESUME at all. */
	...
	func aResumeTimeoutAfterTheAcceptIsIgnored() throws {
		...
		#expect(transfer.resumeTimeoutExpired(for: transfer.negotiation.sessionID) == false)
	...
	func anUnansweredResumeTimesOut() throws {
		...
		#expect(transfer.resumeTimeoutExpired(for: transfer.negotiation.sessionID))
