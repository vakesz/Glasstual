...
struct FileTransferStoreTests {
	...
	func allShowsEverything() {
		#expect(
			FileTransferDirectionFilter.all.transfers(in: everything, isSender: \.isSender)
				== everything
		)
	}
	...
	func sendingShowsOnlySenders() {
		#expect(
			FileTransferDirectionFilter.sending.transfers(in: everything, isSender: \.isSender)
				== [sending, alsoSending]
		)
	}
	...
	func receivingShowsOnlyReceivers() {
		#expect(
			FileTransferDirectionFilter.receiving.transfers(in: everything, isSender: \.isSender)
				== [receiving]
		)
	}
	...
	func filterReadsRealTransfers() throws {
		...
		let incoming = try transfer(on: session, filename: "photo.jpg")
		let outgoing = try transfer(on: session, filename: "notes.txt")
		outgoing.isSender = true

		let all = [incoming, outgoing]

		#expect(
			FileTransferDirectionFilter.sending.transfers(in: all, isSender: \.isSender)
				.map(\.uniqueIdentifier) == [outgoing.uniqueIdentifier]
		)
		#expect(
			FileTransferDirectionFilter.receiving.transfers(in: all, isSender: \.isSender)
	...
	func transfersHaveDistinctIdentifiers() throws {
		let session = TestServerSession()
		let first = try transfer(on: session, filename: "photo.jpg")
		let second = try transfer(on: session, filename: "photo.jpg")
	...
	func removingByIdentityKeepsOrder() throws {
		let session = TestServerSession()
		let first = try transfer(on: session, filename: "one.jpg")
		let second = try transfer(on: session, filename: "two.jpg")
		let third = try transfer(on: session, filename: "three.jpg")
	...
	func modelOwnsListState() throws {
		let session = TestServerSession()
		let incoming = try transfer(on: session, filename: "incoming.jpg")
		let outgoing = try transfer(on: session, filename: "outgoing.jpg")
		outgoing.isSender = true
	...
	func rowPresentationIsAValueSnapshot() throws {
		let session = TestServerSession()
		let transfer = try transfer(on: session, filename: "archive.zip")
	...
	func startActionTitleFollowsTheSelection() throws {
		let session = TestServerSession()
		let offered = try transfer(on: session, filename: "offered.jpg")
		let failed = try transfer(on: session, filename: "failed.jpg")
		failed.transferStatus = .recoverableError
		let model = FileTransferList()
		model.add(offered)
		model.add(failed)

		#expect(model.startActionTitle(for: [offered.uniqueIdentifier]) == String(localized: .FileTransfer.acceptTransfer))
		#expect(model.startActionTitle(for: [failed.uniqueIdentifier]) == String(localized: .FileTransfer.retryTransfer))
		...
		#expect(
			model.startActionTitle(for: [offered.uniqueIdentifier, failed.uniqueIdentifier])
				== String(localized: .FileTransfer.startTransfer)
		)
		/* An outgoing offer is started, never "accepted". */
		offered.isSender = true
		#expect(model.startActionTitle(for: [offered.uniqueIdentifier]) == String(localized: .FileTransfer.startTransfer))
	}
