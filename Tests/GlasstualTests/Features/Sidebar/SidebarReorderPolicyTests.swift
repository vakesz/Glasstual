...
struct SidebarReorderPolicyTests {
	/** The drag as the sidebar performs it, end to end.

	 The indices the policy hands back go to `ChatSession`, which takes the dragged
	 ...
	 could agree with the policy while `ChatSession` disagreed with both. The chat session
	...
	private func moving(_ from: Int, before destination: Int, in order: [String]) -> [String] {
		...
		chatSession.sessions = order.map { name in
			let session = TestServerSession()
			session.config.connectionName = name
			return session
		}
		let list = Sidebar()
		...
		return chatSession.sessions.map(\.config.connectionName)
	}
	...
	func nonMovesAreRefused() {
		#expect(SidebarReorderPolicy.move(fromOffsets: IndexSet(integer: 1), toOffset: 1) == nil)
		#expect(SidebarReorderPolicy.move(fromOffsets: IndexSet(integer: 1), toOffset: 2) == nil)
		/* The sidebar carries one selected row at a time, so a drag of several
		 is not something it can mean. */
		#expect(SidebarReorderPolicy.move(fromOffsets: IndexSet([0, 1]), toOffset: 3) == nil)
		#expect(SidebarReorderPolicy.move(fromOffsets: IndexSet(), toOffset: 1) == nil)
	}

	@Test("Channels and one-to-one conversations do not move into each other")
	func channelMoveBoundaries() {
		#expect(
			SidebarReorderPolicy.permitsMove(draggedIsChannel: true, destinationIsChannel: true)
		)
		#expect(
			SidebarReorderPolicy.permitsMove(draggedIsChannel: true, destinationIsChannel: false)
				== false
		)
		#expect(
			SidebarReorderPolicy.permitsMove(draggedIsChannel: false, destinationIsChannel: false)
		)
	}

	/// A filter draws a list the chat session is not in the order of, so an index into
	/// the rows on screen means nothing to it.
	@Test("A filtered sidebar cannot be reordered")
	func filteredSidebarRefusesMoves() {
		let chatSession = ChatSession()
		chatSession.sessions = ["alpha", "beta"].map { name in
			let session = TestServerSession()
			session.config.connectionName = name
			return session
		}
		...
		#expect(chatSession.sessions.map(\.config.connectionName) == ["alpha", "beta"])
	}
}
