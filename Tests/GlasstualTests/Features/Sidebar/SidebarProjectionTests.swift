...
struct SidebarProjectionTests {
	private let list = Sidebar()
	private let alpha: TestServerSession
	private let beta: TestServerSession
	private let alphaChannels: [Conversation]
	private let betaChannels: [Conversation]
	...
	init() {
		...
		let alpha = TestServerSession()
		let beta = TestServerSession()
		...
		alpha.conversationList = alphaChannels
		beta.conversationList = betaChannels
		...
		chatSession.sessions = [alpha, beta]
		list.chatSessionSource = { chatSession }
		list.filterText = ""
	}

	private static func channel(named name: String, on session: ServerSession) -> Conversation {
		let channel = Conversation(config: ConversationConfig(name: name))
		channel.associatedSession = session
		return channel
	}

	/** The index space is cached, and the cache is dropped the moment the chat session
	 changes rather than when the rows are next rebuilt: the chat session adds a
	...
	func newConversationsAreSelectableBeforeTheRowsAreRebuilt() {
		...
		alpha.conversationList = alphaChannels + [added]
	...
	func rowsCarryTheWholeTree() throws {
		...
		#expect(alphaRow.conversations.map(\.title) == ["#swift", "#coffee"])
		...
		#expect(betaRow.conversations.map(\.title) == ["#swiftui"])
	...
	func filterNarrowsTheListedRows() throws {
		...
		#expect(try #require(list.rows.first).conversations.map(\.title) == ["#coffee"])
	...
	func filterLeavesTheSelectableItemsAlone() throws {
		...
		#expect(list.selectableItems.count == 4)
		#expect(list.row(forItem: openChannel) == rowBeforeFiltering)
		#expect(list.selectableItems[rowBeforeFiltering] === openChannel)
	}
	...
	func filteredMatchesUnderClosedServersAreSelectable() throws {
		...
		#expect(try #require(list.rows.last).conversations.map(\.title) == ["#swiftui"])
		let row = list.row(forItem: hidden)
		#expect(row >= 0)
		#expect(list.selectableItems[row] === hidden)
	...
	func unreadBadgeColourIsPartOfTheRow() throws {
		...
		key.storedValue = SettingsColor(NSColor(srgbRed: 0.9, green: 0.2, blue: 0.1, alpha: 1))
		list.filterText = ""
		let tinted = try #require(list.rows.first?.conversations.first)
		#expect(tinted.unreadBadgeTint != nil)

		key.storedValue = nil
		list.filterText = ""
		let untinted = try #require(list.rows.first?.conversations.first)
