...
private final class TranscriptMediaFixture {
	...
	init() {
		session = ServerSession(config: ServerConfig())
		window = MainWindow(contentRect: .zero, styleMask: .borderless, backing: .buffered, defer: false)
		controller = TranscriptController(
			session: session, in: window,
			inlineImageLoader: InlineImageLoader(budget: budget, protocolClasses: [InlineImageTestProtocol.self])
		)
...
struct TranscriptControllerInlineMediaTests {
	...
	func retiredPreviewReleasesBudget(retirement: TranscriptMediaRetirement) async throws {
		...
		case .trim:
			...
			fixture.controller.print(ChatLine())
