...
struct TranscriptRendererTests {
	...
	func attributedRenderingRemovesControlCharactersAndPreservesEffects() throws {
		...
		let rendered = TranscriptFormattedText.render(
			source,
			with: TranscriptFormattingAttributes(preferredFont: font)
		)
	...
	func colorHelpersMapToNativeColors() {
		#expect(MircColorPalette.mapColorCode(4) == NSColor.formatterColors[4])
		#expect(TranscriptFormattedText.mapColor(NSNumber(value: 4)) == NSColor.formatterColors[4])
		#expect(TranscriptFormattedText.mapColor("4") == nil)
	}
