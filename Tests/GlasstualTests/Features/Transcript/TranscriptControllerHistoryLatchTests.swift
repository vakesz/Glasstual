...
private actor FirstFetchGate {
	...
	private var parked: CheckedContinuation<ScrollbackFetchOutcome, Never>?
	...
	func fetch() async -> ScrollbackFetchOutcome {
		fetches += 1
...
struct TranscriptControllerHistoryLatchTests {
	...
	/** The controller holds its session weakly, so a view can outlive the item
	...
	func reloadWithoutAnItemDoesNotLatchTheFlag() async throws {
		var session: ServerSession? = ServerSession(config: ServerConfig())
		let window = window()
		let controller = try TranscriptController(session: #require(session), in: window)
		controller.loadsHistoryLazily = { false }
		defer { controller.tearDown(.permanentRemoval) }
		...
		session = nil
		try #require(retiredSession == nil)
	...
	func retryAfterRetirementRepairsTheStore() async throws {
		...
		let controller = TranscriptController(
			session: session, in: window, inlineImageLoader: InlineImageLoader(),
	...
	/// and the session would admit a request for that channel. A server console
	...
	func serverHistoryRetryAvailability() {
		...
		let channel = Conversation(config: ConversationConfig(name: "#retry"))
		...
		let console = TranscriptController(session: session, in: window)
