...
struct InlineImageLoaderTests {
	...
	func compressedBombs() async throws {
		...
		await #expect(throws: InlineImageError.self) {
			try await InlineImageDecoder.prepare(enormous, limits: .init())
		}
		await #expect(throws: InlineImageError.self) {
			try await InlineImageDecoder.prepare(Self.gif(frames: 121), limits: .init())
		}
		var limits = InlineImageLimits()
		limits.maximumDecodedBytes = 400 // Each frame fits, but two full canvases do not.
		await #expect(throws: InlineImageError.self) {
			try await InlineImageDecoder.prepare(Self.gif(frames: 2), limits: limits)
		}
		...
		await #expect(throws: InlineImageError.self) {
			try await InlineImageDecoder.prepare(compressed, limits: limits)
		}
	}
	...
	func checkedArithmetic() throws {
		...
		#expect(throws: InlineImageError.self) { try limits.footprint(width: Int.max, height: Int.max) }
		#expect(throws: InlineImageError.self) { try limits.footprint(width: Int.max / 8, height: 1) }
		#expect(throws: InlineImageError.self) { try limits.footprint(width: 0, height: 1) }
		#expect(limits.workingByteCount == nil)
		#expect(InlineImageLimits.product(Int.max, 2) == nil)
		#expect(InlineImageBudget().reserve(view: "overflow", bytes: limits.workingByteCount) == nil)
	}
	...
	func invalidBodies() async {
		for data in [Data(), Data("<svg/>".utf8), Self.gif(frames: 2).dropLast(8)] {
			await #expect(throws: InlineImageError.self) {
				try await InlineImageDecoder.prepare(data, limits: .init())
			}
		}
	}
	...
	func exactStreamLimit() async throws {
		...
		let result = try await InlineImageTransfer.download(
			url,
			limits: limits,
			protocolClasses: [InlineImageTestProtocol.self]
		)
	...
	func boundedEncodedOutput() async throws {
		...
		await #expect(throws: InlineImageError.self) {
			try await InlineImageDecoder.prepare(data as Data, limits: limits)
		}
	}

	@Test("Non-HTTP schemes reject synchronously without network work")
	func schemeGate() throws {
		let budget = InlineImageBudget()
		let loader = InlineImageLoader(budget: budget, protocolClasses: [InlineImageTestProtocol.self])
		var rejected = 0
	...
	func concurrentAdmission() async throws {
		...
		defer { InlineImageTestProtocol.remove(url) }
		let budget = InlineImageBudget(maximumRequests: 2, maximumViewRequests: 1)
		let first = InlineImageLoader(budget: budget, protocolClasses: [InlineImageTestProtocol.self])
		let second = InlineImageLoader(budget: budget, protocolClasses: [InlineImageTestProtocol.self])
		var callbacks = 0
	...
	func retainedReservations() async throws {
		...
		defer { InlineImageTestProtocol.remove(url) }
		let budget = InlineImageBudget(maximumEntries: 1)
		let loader = InlineImageLoader(budget: budget, protocolClasses: [InlineImageTestProtocol.self])
	...
	func destruction() async throws {
		...
		defer { InlineImageTestProtocol.remove(url) }
		let budget = InlineImageBudget()
		var loader: InlineImageLoader? = InlineImageLoader(
			budget: budget,
			protocolClasses: [InlineImageTestProtocol.self]
		)
