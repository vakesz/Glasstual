...
struct ChannelModesFeatureTests {
	...
	func clearingLimitAndChangingKeySubmitsTheNewKey() throws {
		...
		withExtendedLifetime(fixture.session) {
	...
	func remainingKeyLengthGatesSubmission() throws {
		...
		model.setMode(.key, enabled: true)

		model.updateSecretKey("abc")
		#expect(model.keyLengthLimit?.remaining == 0)
		#expect(model.fitsKeyLengthLimit)
		#expect(model.keyLengthCaption == nil)

		model.updateSecretKey("abcde")
		#expect(model.keyLengthLimit?.remaining == -2)
		#expect(model.fitsKeyLengthLimit == false)
		#expect(model.keyLengthCaption == "2 bytes too many")

		model.updateSecretKey("abcd")
		#expect(model.keyLengthCaption == "1 byte too many")

		let (_, unlimitedModel) = try makeModel(maximumKeyLength: 0)
		unlimitedModel.updateSecretKey(String(repeating: "x", count: 1000))
		#expect(unlimitedModel.keyLengthLimit == nil)
		#expect(unlimitedModel.keyLengthCaption == nil)
		#expect(unlimitedModel.fitsKeyLengthLimit)
		...
		#expect(graphemeModel.keyLengthLimit?.remaining == -3)
	}

	@Test("Turning off a key remains possible when its saved value exceeds KEYLEN")
	func removingKeyIgnoresItsLength() throws {
		let session = TestServerSession()
		session.supportInfo.processConfigurationData("CHANMODES=beI,k,l,imnpst PREFIX=(ov)@+ KEYLEN=3")
		let channel = try #require(session.findConversationOrCreate("#swift"))
		channel.activate()
		_ = channel.modeInfo?.updateModes("+k existing-key")
		var acceptedModes: ChannelModeContainer?
		let sheet = ChannelModesSheet(channel: channel) { acceptedModes = $0 }
		#expect(sheet.model.fitsKeyLengthLimit == false)

		sheet.model.setMode(.key, enabled: false)
		#expect(sheet.model.fitsKeyLengthLimit)
		#expect(sheet.model.keyLengthCaption == nil)
		sheet.submit()

		let submitted = try #require(acceptedModes)
		#expect(channel.modeInfo?.changeGroups(for: submitted) == [
			ModeChangeGroup(symbols: "-k", parameters: ["existing-key"]),
		])
	}
	...
	func sheetPreservesIdentityCopiedModesAndReportsEdits() throws {
		let session = TestServerSession()
		session.supportInfo.processConfigurationData("CHANMODES=beI,k,l,imnpst PREFIX=(ov)@+ KEYLEN=8")
		let channel = try #require(session.findConversationOrCreate("#swift"))
		channel.activate()
	...
	private func makeModelState(
		...
		let channel = try #require(session.findConversationOrCreate("#test"))
