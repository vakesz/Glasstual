...
struct SettingsArchiveTests {
	...
	private static let laterKeys = [SettingsKeys.Messages.showJoinLeave.name, SettingsKeys.Reactions.recent.name]
	...
	private func olderKeySetDocument(
		_ archive: SettingsArchive, source: SettingsArchive.Source
	) throws -> Data {
		...
		var settings = try #require(root["preferences"]?.dictionary)
		...
		root["preferences"] = .dictionary(settings)
	...
	private func prepare(
		_ data: Data, source: SettingsArchive.Source, session: SettingsTransferSession, fixture: Fixture
	) async throws {
		switch source {
		case .portable:
		...
		case .localRecovery:
			...
			await session.prepareRecovery(SettingsRecoveryBackup(url: url, created: Date()))
		}
	}

	@Test("An archive written before a key was declared still imports, and Merge keeps this Mac's value",
	      arguments: [SettingsArchive.Source.portable, .localRecovery])
	func olderKeySetMergeLeavesLaterKeysAlone(source: SettingsArchive.Source) async throws {
		let fixture = try Fixture()
		defer { fixture.cleanUp() }
		var archive = SettingsArchive.snapshot(from: fixture.stores, sessions: [])
		archive.values[SettingsKeys.Messages.showDateChanges.name] = false
		let data = try olderKeySetDocument(archive, source: source)

		fixture.stores.set(false, for: SettingsKeys.Messages.showJoinLeave)
		fixture.stores.set(.array(["🎉"]), for: SettingsKeys.Reactions.recent)
		...
		#expect(fixture.stores[SettingsKeys.Messages.showDateChanges] == false)
		#expect(fixture.stores[stored: SettingsKeys.Messages.showJoinLeave] == false)
		#expect(fixture.stores[stored: SettingsKeys.Reactions.recent] == ["🎉"])
	}

	@Test("Restoring an archive written before a key was declared returns that key to its registered default",
	      arguments: [SettingsArchive.Source.portable, .localRecovery])
	func olderKeySetRestoreResetsLaterKeys(source: SettingsArchive.Source) async throws {
		let fixture = try Fixture()
		defer { fixture.cleanUp() }
		let data = try olderKeySetDocument(SettingsArchive.snapshot(from: fixture.stores, sessions: []), source: source)
		...
		await session.commitPreview()
		#expect(session.errorMessage == nil)
		#expect(fixture.stores.persistedValue(for: SettingsKeys.Messages.showJoinLeave) == nil)
		#expect(fixture.stores[SettingsKeys.Messages.showJoinLeave] == SettingsKeys.Messages.showJoinLeave.defaultValue)
		#expect(fixture.stores.persistedValue(for: SettingsKeys.Reactions.recent) == nil)
	}
	...
	func restoreComparesEffectiveValues() throws {
		let fixture = try Fixture()
		defer { fixture.cleanUp() }
		let key = SettingsKeys.Messages.showJoinLeave
		let current = SettingsArchive.snapshot(from: fixture.stores, sessions: [])
		var archive = current
		archive.values.removeValue(forKey: key.name)
		let plan = try SettingsTransferPlan(archive: archive, current: current, mode: .restore)
	...
	func mergeDoesNotApplyTheSourcesUnsetKeys() async throws {
		...
		let lists: [(any AnySettingsKey, PropertyListValue)] = [
			(SettingsKeys.Rules.messageRules, .array([.dictionary(["uniqueIdentifier": "kept-filter"])])),
			(SettingsKeys.Highlights.matchKeywords, .array([.dictionary(["string": "kept keyword"])])),
			(SettingsKeys.LinkSchemes.permitted, .array(["kept-scheme"])),
			(SettingsKeys.Input.tabCompletionSuffix, "kept, "),
			(SettingsKeys.Reactions.recent, .array(["🎉"])),
		]
		for (key, value) in lists {
			target.stores.set(value, for: key)
		}
		source.stores.set(false, for: SettingsKeys.Messages.showJoinLeave)
		let exported = try SettingsArchive.decode(SettingsArchive.snapshot(from: source.stores, sessions: []).encoded())
		for (key, _) in lists {
			#expect(exported.unset.contains(key.name))
		}

		let session = target.session()
		try await session.prepareImport(from: target.write(SettingsArchive.snapshot(from: source.stores, sessions: []).encoded()))
		#expect(session.previewMode == .merge)
		#expect(session.preview?.plan?.removedKeys.isEmpty == true)
		await session.commitPreview()
		#expect(session.errorMessage == nil)
		#expect(target.stores[stored: SettingsKeys.Messages.showJoinLeave] == false)
		for (key, value) in lists {
	...
	func archivesDefaultToPortable() {
		let archive = SettingsArchive(values: [:], unset: [], sessions: [])
		#expect(archive.source == .portable)
		#expect(throws: SettingsTransferError.self) { try archive.recoveryEncoded() }
	}
}
