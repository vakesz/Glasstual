...
struct ServerPropertiesNetworkCompletionTests {
	...
	private static func configuration(
		...
		var config = ServerConfig(connectionName: "Test Connection")
		config.nickname = "someone"
		config.username = "someone"
		config.realName = "Someone"
		config.serverList = [
			ServerEndpoint(serverAddress: serverAddress, serverPort: port, prefersSecuredConnection: secured),
		]
	...
	func everyBundledNetworkNameIsAccepted() {
		...
		#expect("Fuel Rats".isValidInternetAddress == false)
	}
}
