...
struct ServerEndpointListSheetTests {
	...
	func aRefusedRowNamesItsFault() {
		...
		model.replace(with: [
			ServerEndpoint(serverAddress: "not a host", serverPort: 6667),
			ServerEndpoint(serverAddress: "irc.example.com", serverPort: 6667),
		])
	...
	func editorsWriteThroughByIdentity() {
		let model = ServerEndpointListModel()
		model.replace(with: [ServerEndpoint(serverAddress: "irc.example.com", serverPort: 6667)])
	...
	func securingMovesTheDefaultPort() {
		let server = ServerEndpoint(serverAddress: "irc.example.com", serverPort: 6667)
	...
	func aChosenPortSurvivesTheTransportChange(port: UInt16) {
		let server = ServerEndpoint(serverAddress: "irc.example.com", serverPort: port)
	...
	func switchingTransportKeepsIdentity() {
		var server = ServerEndpoint(serverAddress: "irc.example.com", serverPort: 6667)
	...
	func entriesRoundTrip() throws {
		let model = ServerEndpointListModel()
		model.replace(with: [
			ServerEndpoint(serverAddress: "irc.one.example", serverPort: 6667),
			ServerEndpoint(serverAddress: "irc.two.example", serverPort: 6667),
		])
	...
	func unflushedEndpointEditsSurviveUntouched() throws {
		let model = ServerEndpointListModel()
		model.replace(with: [
			ServerEndpoint(serverAddress: "irc.one.example", serverPort: 6667, pendingServerPassword: .set("hunter2")),
			ServerEndpoint(serverAddress: "irc.two.example", serverPort: 6667, pendingServerPassword: .cleared),
		])
	...
	func entryIdentifiersAreDistinct() {
		let entries = [ServerEndpoint(), ServerEndpoint(), ServerEndpoint()]
	...
	func reorderingMovesOneEndpoint() {
		let model = ServerEndpointListModel()
		model.replace(with: [
			ServerEndpoint(serverAddress: "one"),
			ServerEndpoint(serverAddress: "two"),
			ServerEndpoint(serverAddress: "three"),
		])
	...
	func blankEndpointsAreRejected() {
		...
		model.replace(with: [
			ServerEndpoint(serverAddress: "irc.example.com", serverPort: 6667),
			ServerEndpoint(serverAddress: "", serverPort: 6667),
		])
	...
	func nativeEditorSubmitsValidRowsInOrder() throws {
		let model = ServerEndpointListModel()
		model.replace(with: [
			ServerEndpoint(serverAddress: "irc.one.example", serverPort: 6697, prefersSecuredConnection: true),
			ServerEndpoint(serverAddress: "irc.two.example", serverPort: 6667),
		])
	...
	func nativeEditorReportsInvalidFields() {
		let model = ServerEndpointListModel()
		model.replace(with: [ServerEndpoint(serverAddress: "not a host", serverPort: 6667)])
		let id = model.entries[0].id
