...
struct ServerPropertiesSheetTableTests {
	private func makeChannel(named name: String, type: ConversationKind) -> ConversationConfig {
		var config = ConversationConfig(name: name)
		config.type = type

		return config
	}

	private func displayedChannels(in conversationList: [ConversationConfig]) -> [ConversationConfig] {
		var config = ServerConfig()
		config.conversationList = conversationList
	...
	func orderIsPreserved() {
		...
		#expect(displayed.map(\.name) == ["#c", "#a", "#b"])
	}
