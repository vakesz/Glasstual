...
struct ServerPropertiesTemplateTests {
	...
	private static func newConnectionModel() throws -> ServerPropertiesModel {
		try ServerPropertiesModel(config: ServerConfig(), networkList: catalog(), offersTemplates: true)
	}
	...
	func onlyANewConnectionOffersTemplates() throws {
		...
		let existing = try ServerPropertiesModel(config: ServerConfig(), networkList: Self.catalog())
	...
	func networkFillsTheFormIn() throws {
		...
		#expect(model.config.conversationList.map(\.name) == ["#libera", "#swift"])
		#expect(model.config.conversationList.map(\.autoJoin) == [true, true])
	}
	...
	func plainNetworkLeavesSASLAndChannelsOff() throws {
		...
		#expect(model.config.conversationList.isEmpty)
	}
	...
	func appliedTemplateMatchesTypedNetwork() throws {
		let typed = try ServerPropertiesModel(config: ServerConfig(), networkList: Self.catalog())
