...
@Suite("Message rule engine")
@MainActor
struct MessageRuleMatcherTests {
	...
	func remoteSeparatorsCannotStartACommand(_ separator: (name: String, scalar: String)) {
		let commands = MessageRuleMatcher.actionCommands(
	...
	func unknownTokenExpandsToNothing() {
		#expect(
			MessageRuleMatcher.actionCommands(in: "/say [%_channelName_%]", replacing: ["%_channelName_%": ""])
				== ["say []"]
		)
	}
