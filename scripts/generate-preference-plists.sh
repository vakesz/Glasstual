...
#
# The declarations import CocoaExtensions but use nothing from it, so the tool
# compiles against an empty module of that name rather than against the built
# framework. That keeps this a few seconds of `swiftc` instead of a nested
# build, which matters because it runs as a build phase of the application.
#
# Usage
...
stamp="$(shasum -a 256 "${sources[@]}" | shasum -a 256)"
tool="$build_dir/generate-preference-plists"

if [ ! -x "$tool" ] || [ "$(cat "$build_dir/inputs.sha256" 2>/dev/null)" != "$stamp" ]; then
	rm -rf "$build_dir/CocoaExtensions.swiftmodule"
	printf '// The declarations import this module but use nothing from it.\n' \
		> "$build_dir/CocoaExtensions.swift"

	if ! xcrun swiftc -swift-version 6 -O -module-name CocoaExtensions \
		-emit-module -emit-module-path "$build_dir/CocoaExtensions.swiftmodule" \
		"$build_dir/CocoaExtensions.swift"; then
	...
	if ! xcrun swiftc -swift-version 6 -O -module-name PreferencePlistGenerator \
		-I "$build_dir" -o "$tool" "${sources[@]}"; then
