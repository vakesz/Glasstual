...
.PHONY: help setup generate project-check build release archive run test lint format format-check ensure-xcodegen ensure-formatters ensure-linters clean
...
project-check: ensure-xcodegen ## Verify Glasstual.xcodeproj matches project.yml
	@temp_dir="$$(mktemp -d)"; trap 'rm -rf "$$temp_dir"' EXIT; \
		xcodegen generate --spec project.yml --project "$$temp_dir" --project-root "$(CURDIR)" --quiet; \
		cmp Glasstual.xcodeproj/project.pbxproj "$$temp_dir/Glasstual.xcodeproj/project.pbxproj"; \
		for scheme in Glasstual.xcodeproj/xcshareddata/xcschemes/*.xcscheme; do \
			cmp "$$scheme" "$$temp_dir/$$scheme"; \
		done
